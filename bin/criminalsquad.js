...
import { promoverSkillCli } from '../src/skill-promotion.js';
...
import { ativarCli, statusCli, syncCli } from '../src/sync-cli.js';

const HELP = `
  criminalsquad — Multi-agent orchestration for Claude Code and Cursor
  ...
  Usage:
    ...
    npx criminalsquad install-global          Install for ALL Claude Code + Cursor
                                              conversations (~/.claude and ~/.cursor)
    ...
    npx criminalsquad promover-skill <name>   Scaffold promotion evidence (born failing, by design)
    npx criminalsquad promover-skill --conferir <file>   Check evidence against the installed skill
    ...
    npx criminalsquad ativar <token>          Ativar a licenca (CSQ-XXXX-XXXX-XXXX)
    npx criminalsquad sync [--from <dir>]     Baixar o conteudo novo da curadoria
                          [--timeout <ms>]   Teto por pacote em link ruim (padrao 10 min)
    npx criminalsquad status                  Titular, licenca e pacotes sincronizados
...
const { positionals, values } = parseArgs({
  ...
  options: {
    ...
    from: { type: 'string' },
    check: { type: 'boolean' },
    conferir: { type: 'string' },
    status: { type: 'string' },
    saida: { type: 'string' },
    // Declarado como string para que `--timeout 60000` (com espaço) chegue com o
    // valor. Sem isto, `strict: false` entrega `true` e o teto viraria 1 ms —
    // e esta é justamente a saída de emergência de quem está com link ruim.
    timeout: { type: 'string' },
  },
});
...
const commands = {
  ...
  'promover-skill': {
    run: () => promoverSkillCli(positionals.slice(1), cwd, values),
    checkSuccess: true,
  },
  'search-skills': {
  ...
  ativar: { run: () => ativarCli(positionals.slice(1), values), checkSuccess: true },
  sync: { run: () => syncCli(cwd, values), checkSuccess: true },
  status: { run: () => statusCli(cwd), checkSuccess: true },
  'resolve-skills': {
