...
// Quantas skills existem NO DISCO. O número era fixo (487) e ficou defasado
// silenciosamente enquanto a biblioteca crescia — um contador que não acompanha
// o repo deixa de ser invariante e vira ruído. Derivar do disco mantém a
// comparação honesta: o que existe aqui tem que ser exatamente o que embarca.
function skillsNoDisco() {
...
// Os 7 módulos Python do subsistema de captura audiovisual (assistir vídeo /
// transcrever áudio dos autos). Sem eles no tarball, `criminalsquad captura`
// quebra na máquina do advogado com ImportError.
const MODULOS_CAPTURA = [
  'config.py',
  'download.py',
  'frames.py',
  'providers.py',
...
const checks = [
  ...
  // higiene do tarball: sigilo/LGPD + distribuição correta
  {
    ...
    fn: () => {
      ...
      // Bytecode Python: além de peso morto, cada .pyc carrega embutido o caminho
      // ABSOLUTO da máquina onde foi compilado (nome de usuário, estrutura de
      // pastas do dev) e o número mágico de uma versão específica do CPython.
      const bytecode = files.filter((f) => /__pycache__|\.pyc$/.test(f));
      if (bytecode.length) {
        problems.push(
          `VAZAMENTO: ${bytecode.length} arquivo(s) de bytecode Python no tarball ` +
          `(ex.: ${bytecode[0]}) — cada .pyc embute o caminho absoluto da máquina do dev`,
        );
      }
      ...
      const totalSkills = skillsNoDisco().length;
      const empacotadas = count(/^skills\/[^/]+\/SKILL\.md$/);
      if (empacotadas !== totalSkills) problems.push(`o tarball leva ${empacotadas} SKILL.md, mas existem ${totalSkills} skills em skills/`);
      if (count(/^skills\/[^/]+\/references\/high-performance-contract\.md$/) !== totalSkills) problems.push(`faltam contratos progressivos: ${count(/^skills\/[^/]+\/references\/high-performance-contract\.md$/)} de ${totalSkills} skills`);
      if (count(/^skills\/[^/]+\/agents\/openai\.yaml$/) !== totalSkills) problems.push(`faltam metadados OpenAI: ${count(/^skills\/[^/]+\/agents\/openai\.yaml$/)} de ${totalSkills} skills`);
      ...
      // Captura audiovisual: o CLI é JS, mas quem faz o trabalho é o Python.
      for (const modulo of MODULOS_CAPTURA) {
        if (!count(new RegExp(`^scripts/captura/${modulo.replace('.', '\\.')}$`))) {
          problems.push(`faltando scripts/captura/${modulo} no tarball (captura quebra no aluno)`);
        }
      }
      // A licença MIT do claude-video precisa viajar junto com o código derivado dele.
      if (!count(/^scripts\/captura\/LICENSE\.claude-video$/)) problems.push('faltando scripts/captura/LICENSE.claude-video no tarball (obrigação da licença MIT de origem)');
      if (!count(/^skills\/captura-midia-av\/SKILL\.md$/)) problems.push('faltando a skill captura-midia-av no tarball');
      try {
