...
FONTE="$(git rev-parse --short HEAD)"
OUT="$(mktemp -d)/criminalsquad-dist"

# GUARDA — o build-dist copia da ÁRVORE DE TRABALHO, não do HEAD. A lógica vive
# em Node para poder ser testada (ver tests/publish-dist-guarda.test.js).
node scripts/check-arvore-para-dist.mjs

echo "==> Construindo a árvore de distribuição (v${VERSION}, fonte ${FONTE})…"
...
git add -A
# O SHA de origem no assunto é o que distingue dois snapshots publicados sob a
# MESMA versão — aconteceu três vezes com a v0.3.0, e sem ele nada no repo
# público dizia qual conteúdo o aluno tinha instalado.
git commit -q -m "dist: criminalsquad v${VERSION} (fonte ${FONTE})"
git branch -M main
