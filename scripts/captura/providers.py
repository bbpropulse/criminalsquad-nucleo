...
- backend "local" (PADRAO): faster-whisper na maquina. Nenhum byte de audio sai
  ...
  justica (sigilo profissional + LGPD). Instala no uso com `npx criminalsquad captura
  setup`, que cuida tambem do Python "gerenciado pelo sistema" (PEP 668) criando um
  ambiente proprio — resolvido aqui por _ensure_faster_whisper_importable().
...
# ------------------------------------------- ambiente proprio do transcritor local
# ESPELHO de setup.py (_config_root/_venv_dir/_venv_site_packages). Duplicado de
# proposito: providers.py e importado pelo watch.py em todo uso e nao pode depender
# do instalador. Mudou aqui, mude la.
...
def _captura_venv_site_packages() -> Path | None:
    """site-packages do venv do captura, SO se casar com o Python que esta rodando."""
    venv = _config_root() / "criminalsquad" / "captura-venv"
    if os.name == "nt":
...
def _ensure_faster_whisper_importable() -> None:
    """Torna o faster-whisper importavel mesmo quando ele mora no venv do captura.

    POR QUE injetar sys.path em vez de chamar o python do venv como subprocesso:
    o venv e criado pelo setup.py com `<este mesmo python> -m venv`, logo a versao e
    a ABI sao identicas e as extensoes C (ctranslate2, numpy) carregam sem conflito —
    a checagem de versao em _captura_venv_site_packages garante isso. Um subprocesso
    exigiria serializar audio e segmentos entre processos e duplicar o tratamento de
    erro/lacunas de _transcribe_chunks, sem ganho nenhum. Se a versao do Python mudar,
    o caminho versionado deixa de existir, nada e injetado e o usuario recebe a
    instrucao de rodar o setup de novo (que refaz o venv na versao certa).
    """
    site_packages = _captura_venv_site_packages()
    if site_packages is None:
        return
    path = str(site_packages)
    if path not in sys.path:
        # Na frente: o venv foi resolvido pelo pip como um conjunto coerente e deve
        # vencer qualquer numpy/onnxruntime solto no Python do sistema.
...
def transcribe_local(audio_path: Path) -> list[dict]:
    ...
    _ensure_faster_whisper_importable()
    try:
        from faster_whisper import WhisperModel
    except ImportError:
        raise SystemExit(
            "captura: o transcritor local (faster-whisper) ainda nao esta instalado. "
            "Rode uma vez: npx criminalsquad captura setup"
        )
...
def transcribe_video(
    ...
    # Lista vazia NAO e falha: um audio sem fala (musica, ruido ambiente de CFTV,
    # silencio) transcreve com SUCESSO e resultado vazio. Tratar isso como erro
    # (SystemExit) fazia um audio ambiental legitimo virar "transcricao falhou" +
    # exit 1. A falha real (deps ausentes, modelo quebrado) ja sobe como excecao
    # de transcribe_local/transcribe_openrouter, antes de chegar aqui.
    if not segments:
        print("[captura] transcricao concluida: nenhuma fala detectada no audio", file=sys.stderr)
    else:
