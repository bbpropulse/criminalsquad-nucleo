...
CriminalSquad é um framework de orquestração multi-agente focado em **Direito Penal e Processual Penal** e na gestão do escritório/gabinete. Já vem equipado com subagentes jurídicos especialistas, **521 skills com contrato v5 auditável**, best-practices de domínio (inclusive **por nicho**), um **acervo de conhecimento local** e squads jurídicos prontos — e ainda permite criar novos squads, agentes e peças sob demanda (via **Arquiteto**), descrevendo o que você precisa em português. **Toda entrega é um rascunho técnico para revisão humana obrigatória.**
...
Skills, acervo, agentes e squads novos entram em `curadoria/`. Para revisar os
gates e publicar os snapshots com um token exclusivo de curadoria:

```bash
npm run curadoria:app
```

O aplicativo abre somente na máquina local, consulta o que já está no ar e envia
os pacotes ao webserver. O token não é o `ADMIN_TOKEN` e não vai para os alunos.
Veja o procedimento completo em
[`curadoria/README.md`](curadoria/README.md).
...
O critério é técnico, não comercial. A orquestração depende de **três** recursos, e uma IDE só entra quando tem os três:

1. **subagente em contexto isolado** — o revisor que não é o redator, o juiz do smoke-run;
2. **hook determinístico que bloqueia antes da escrita** — o Citation Gate, que impede a peça de chegar ao disco com citação não verificada;
3. **skills** (`SKILL.md`).

Sem subagente não há orquestração: seria entregar a casca e chamar de suporte.

| IDE | Status |
|-----|--------|
| Claude Code | **Disponível** |
| Cursor (2.4+) | **Disponível** — subagentes, `hooks.json` e skills |
| VS Code + Copilot, Codex, Open Code, Antigravity, Gemini CLI, Qwen Code, Trae | Não suportadas — sem subagente em contexto isolado |
...
A descoberta também é progressiva: `search-skills` consulta localmente as 521 entradas e devolve no máximo uma shortlist compacta, sem carregar os 597 KB do índice completo nem os bodies das skills no prompt.
...
- **Moldura ética conforme a instituição.** OAB + Provimento 205/2021 (advocacia), CNMP (Ministério Público), LC 80/94 (Defensoria). Conflito de interesses (art. 15, § 6º, EAOAB + CED 19-21) é checado na triagem.
