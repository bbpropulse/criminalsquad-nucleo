...
1. **Preflight compacto de skills (sempre primeiro) — BUSCA EM LEQUE, nunca uma consulta só.** Reduza o propósito a termos de capability, sem nomes, fatos ou dados do caso. Derive **3 a 6 termos-âncora** e rode **uma busca por termo**, unindo os resultados:
   - **lei especial** (ex.: "Lei 8.137", "Maria da Penha", "Lei 9.605")
   - **tipo penal / crime** (ex.: "sonegação", "descaminho", "lesão corporal")
   - **peça** (ex.: "queixa-crime", "habeas corpus", "medida protetiva")
   - **fase** (ex.: "inquérito", "execução penal", "recurso especial")
   - **tese / instituto** (ex.: "extinção da punibilidade", "prescrição")

   Uma consulta só recupera parte do que existe: medido no acervo, a busca única
   trouxe 3 de 5 skills em tributário e 6 de 7 em protetivas; o leque trouxe 5/5 e
   7/7. **O acervo já tem o conteúdo — quem não o extrai é a consulta única.** E o
   preço do recall baixo é caro: sem ver a cobertura, o Arquiteto declara lacuna e
   manda criar skill **duplicada**.

   Cada busca: `npx criminalsquad search-skills --query "<termo-âncora>" --limit 8 --json`. O motor lê o catálogo local e devolve somente lifecycle, qualidade, risco, perfil, gatilhos e limites dos candidatos; **não leia `skills/_index.yaml` por inteiro**. Para evolução arquitetural — nunca execução — repita com `--include-preview` e consulte por `rg` apenas as linhas necessárias do manifesto `skills/_execucao-penal-v3-integration.yaml`. Se a busca falhar por índice ausente/stale, sinalize `check-skills`/`indexar-skills`; `rg` direcionado é fallback degradado.
