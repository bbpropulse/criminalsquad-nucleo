...
- **Prova audiovisual exige transcrição prévia.** Se o depoimento/interrogatório está em áudio ou vídeo, **não opine sobre o conteúdo** sem transcrição — acione a skill `captura-midia-av` (transcrição ancorada por timestamp + frames) ou peça a degravação oficial. Trabalhar de "ouvido" gera erro grave.
...
Q3: "Há prova em áudio/vídeo? Existe transcrição/degravação? (se não, aciono `captura-midia-av`)"
...
- Achados prontos para virar peça → `defesa-criminal-resposta-acusacao`, `habeas-corpus`, `apelacao-criminal` ou memorial. Súmulas/precedentes a validar → `verificacao-citacoes` (hoje: `jurisprudencia-stj-stf`). Texto literal de lei/súmula → `lei-e-sumula`. Base factual / linha do tempo → `resumo-processo` (reaproveitar, não refazer). Transcrição de áudio/vídeo (audiência, depoimento, CFTV, bodycam) → skill `captura-midia-av`; PDF escaneado dos autos → skill `ocr-autos-pdf`.
