...
3. **Classifique cada citação:**
   ...
   - **ACESSO FALHOU** — a fonte oficial não abriu (portal fora do ar, timeout, erro de rede). **Não é VERIFICADA**, e também não acusa a citação de inventada: é uma conferência que não aconteceu. No manifesto do Citation Gate corresponde a `status: "acesso_falhou"`; a citação **sai da peça** até alguém abrir a fonte.
...
Feche com: **contagem** (verificadas/divergentes/não encontradas/acesso falhou) e um **veredito geral**: `APROVADO` (todas verificadas) ou `Revisão humana necessária` (há divergente, não encontrada ou acesso falhou). Em Revisão humana necessária, instrua o redator a **marcar cada citação problemática com `[NÃO VERIFICADO]` ou `[DIVERGENTE]`** e corrigir/remover — o hook `verifica-citacoes` bloqueia a finalização enquanto restar marcador.
...
- Dar VERIFICADA porque a fonte **não abriu** — isso é ACESSO FALHOU. Foi o defeito medido numa peça real: o manifesto atestou "verificada" para uma URL que devolveu erro de conexão.
