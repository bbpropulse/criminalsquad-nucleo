---
name: criminalsquad
description: "CriminalSquad — Orquestração multi-agente para advocacia criminal. Cria e executa squads jurídicos: defesa, recursos, prazos/intimações, triagem de clientes e conteúdo de autoridade."
---
...
4. **Primeira ativação nesta pasta** (o onboarding disparou, ou você acabou de rodar o `init` automático): rode `npx criminalsquad status` com a ferramenta Bash e **repasse as linhas de licença e de pacotes como elas saem**. Ele já imprime "Sem licenca ativa. Ative com: …" e "O conteudo inicial embarcado nao depende de sync" — aproveite esse texto em vez de escrever outro. É a única vez em que o aluno descobre que existe curadoria para baixar; sem isto ele usa o produto por meses sem saber. Só informe: não peça o token nem insista.
...
| `/criminalsquad ativar <CSQ-XXXX-XXXX-XXXX>` (ou "recebi meu token", "ativar a licença") | Gravar a licença do Núcleo — ver "Curadoria (conteúdo novo)" |
| `/criminalsquad sync` (ou "baixar a curadoria", "chegou material novo?", "tem conteúdo novo?") | Baixar a curadoria publicada depois da instalação — ver "Curadoria (conteúdo novo)" |
| `/criminalsquad status` (ou "minha licença", "estou em dia?", "o que já baixei?") | Licença, pacotes sincronizados e frescor — ver "Curadoria (conteúdo novo)" |
| `/criminalsquad atualizar` (ou "atualizar o criminalsquad", "tem versão nova?") | Atualizar o framework **e** buscar a curadoria — ver "Atualizar o CriminalSquad" |
...
São **duas coisas diferentes**, e a frase do aluno quase nunca distingue: os passos 1-3 trocam o **framework** (o motor, os agentes e o conteúdo que veio embarcado no pacote npm); o passo 4 traz a **curadoria** (o material publicado depois da instalação). Faça os quatro e diga qual entregou o quê.
...
4. **Buscar a curadoria:** rode `npx criminalsquad sync`. Nada nos passos 1-3 traz conteúdo novo da curadoria — eles reinstalam o pacote npm, e o material publicado depois da instalação (skills novas, jurisprudência, teses) só chega por aqui, em `_criminalsquad/_packs/`. Sem este passo, a pergunta mais natural do aluno ("tem versão nova?") recebe um "tudo atualizado" sem que uma linha de curadoria tenha sido entregue.

Then read the output and report in plain Portuguese what changed. **Duas linhas, uma para cada coisa:** *framework* — a versão que passou a valer e o que foi atualizado no projeto; *curadoria* — quais pacotes entraram (ou "nada novo publicado desde a última vez"). Reassure the user that their own content is preserved: `_criminalsquad/_memory/`, `acervo/` (seus materiais e o índice) e `squads/` não são tocados; qualquer outro arquivo que eles tenham alterado é salvo como `.bak` antes de ser atualizado.

If step 1 fails (no repo access / offline), explain simply and still run `criminalsquad update` (refreshes from the version already installed). Never expose npm jargon in your reply. Para as recusas do passo 4, siga a regra de fail-closed da seção "Curadoria (conteúdo novo)" — não trate pacote recusado como falha passageira.

## Curadoria (conteúdo novo)

O conteúdo que veio na instalação **funciona sem licença e sem internet** — as skills, o acervo e os squads embarcados são o produto desde o dia 1. A licença do Núcleo só acrescenta a **curadoria publicada depois**: skills novas, jurisprudência e teses da semana. Diga isso sempre que o assunto for licença; o aluno sem token não está com o sistema pela metade, e nada para de funcionar quando a licença vence.

| O aluno diz | O que fazer |
|---|---|
| "chegou material novo?", "baixar a curadoria" | rode `npx criminalsquad sync` |
| "minha licença", "estou em dia?", "o que já baixei?" | rode `npx criminalsquad status` |
| "recebi meu token", "ativar a licença" | **peça para ele rodar no terminal dele:** `npx criminalsquad ativar CSQ-XXXX-XXXX-XXXX` |

Os dois primeiros você roda, com a ferramenta Bash, na raiz do projeto — o aluno nunca digita npm/npx à mão. Depois traduza a saída para linguagem simples. O `sync` já reindexa sozinho ao final, então o que entrou **já aparece na busca**: não mande o aluno reindexar nada.

O `ativar` é a exceção, por dois motivos: ele **pergunta nome e CPF** no terminal (esperando resposta — rodado por ferramenta, trava ou grava licença vazia), e **CPF é dado pessoal que não deve passar por esta conversa**. Passe o comando e deixe o aluno digitar. Diga só o que ele precisa saber: pergunta uma vez, e o CPF **não é gravado** — ficam a máscara de exibição e o hash. Depois que ele avisar que ativou, aí sim rode o `sync`.

**Fail-closed — não suavize.** Pacote com assinatura inválida, com `pack_id` diferente do que o catálogo prometeu, ou licença revogada (403) e token inválido (401): o pacote é **recusado** e **nada é gravado**. Reporte a recusa como ela veio e pare. Não repita o comando "para ver se passa" — repetir não conserta assinatura, e um pacote recusado é sinal de adulteração no caminho, não de instabilidade. **Sem rede é outra coisa:** aí não houve recusa nenhuma, o conteúdo local continua servindo e basta tentar mais tarde — o próprio comando já diz isso.
...
3. Proceed to Phase 5 (Smoke-run) — do **not** present the squad as ready yet.

### Phase 5: Smoke-run (o squad estreia no caso-ouro, não no caso real)

Um squad recém-criado passou só por gates **estruturais**: os arquivos existem e
têm o formato certo. Ninguém sabe se ele **funciona**. Sem esta fase, a estreia é
no caso real do cliente — com prazo correndo. Aqui ele roda uma vez em ambiente
seguro e vira a primeira linha de evidência comportamental.

1. **Rode o squad sobre o caso-ouro fictício** que o Build gerou em
   `squads/{code}/_evals/casos/`, no **Modo smoke** — a seção com esse nome em
   `_criminalsquad/core/runner.pipeline.md` define as regras. Em resumo:
   - o checkpoint **não é pulado**: é *respondido a partir do caso-ouro* (pular
     violaria "nunca passe de um checkpoint sem o usuário", que continua valendo);
   - checkpoint que antecede **ato irreversível** (protocolar, enviar, publicar)
     **PARA ali** — nada é protocolado num teste;
   - `run_id` prefixado `smoke-`, para a saída não se misturar ao trabalho real;
   - dados fictícios, então não há sigilo nem prazo real em jogo;
   - se o caso-ouro não existir, crie-o antes (formato em
     `squads/defesa-criminal-completa/_evals/casos/exemplo-furto-noturno.md`).

2. **Avalie com o juiz isolado.** Despache o subagente `avaliador-squad`
   (read-only, contexto isolado — anti-viés) com o output do smoke-run e o
   `squad.yaml`. Ele pontua cada `success_criteria`
   ATENDE(2)/PARCIAL(1)/NÃO(0) e devolve `nota` 0–100 + `verdict`.

3. **Registre em `squads/{code}/_evals/scores.md`** — uma linha na tabela:
   `| {data} | smoke-run/{caso} | {nota} | {verdict} | {1 linha} |`.
   É esta linha que permite pegar **regressão** quando o squad for editado depois.

4. **Regra de entrega (BLOQUEANTE):** se `nota < 80` **ou** qualquer critério
   ficar em **NÃO ATENDE**, o squad **não** é declarado pronto. Apresente ao
   usuário as sugestões do avaliador e ofereça **1 ciclo de correção**; depois do
   ciclo, reavalie. Persistindo a reprovação, entregue com o diagnóstico
   explícito do que não pôde ser garantido — nunca anuncie "squad pronto" sobre
   um squad que reprovou no próprio caso-ouro.

5. Só então apresente o resumo de conclusão ao usuário, **com a nota**.
...
- Build complete (`squad.yaml` existe) mas `_evals/scores.md` sem nenhuma linha
  → resume from Phase 5 (o squad ficou sem smoke-run)
...
CURADORIA (conteúdo novo)
  /criminalsquad ativar <token>   Ativar a licença do Núcleo (o conteúdo embarcado já funciona sem ela)
  /criminalsquad sync             Baixar a curadoria publicada depois da instalação
  /criminalsquad status           Licença, pacotes já sincronizados e frescor

MANUTENÇÃO
  /criminalsquad atualizar        Atualizar o framework e buscar a curadoria
