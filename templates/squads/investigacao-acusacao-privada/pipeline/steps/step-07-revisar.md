...
- [ ] **MANIFESTO VÁLIDO NO FECHAMENTO:** `node .claude/hooks/verifica-citacoes.mjs --check "$PECA"` sai sem bloqueio (exit 0) — `output/peca.md.citation-gate.json` existe e seu `artifact_sha256` corresponde à peça **desta** versão. Se a peça foi regravada no loop, o manifesto tem de ter sido regravado junto (Step 05); manifesto com hash velho = pipeline travado, não peça pronta.
...
- O Quim Queixa aplica APENAS as correções indicadas — e, como a peça foi regravada, **recalcula o SHA-256 e regrava `output/peca.md.citation-gate.json`** (o manifesto do ciclo anterior atesta um arquivo que não existe mais).
