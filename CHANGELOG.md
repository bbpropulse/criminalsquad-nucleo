...
### Added
- **Gate de Redação — a escrita virou critério verificável, não só instruída.** O defeito veio de um squad que o **Arquiteto montou**, não de um dos prontos: o redator carregava `redacao-persuasiva-criminal`, mas o revisor emitia **um voto só**, então peça bem fundamentada e mal escrita passava com todos os gates verdes. Instruir não é verificar. Três camadas, na ordem "proibição em prosa não segura; motor segura": (1) **hook determinista** `.claude/hooks/verifica-redacao.mjs`, irmão mecânico do Citation Gate, que **bloqueia antes da escrita** (exit 2) a peça que carrega andaime do pipeline — front-matter de controle, marcador `(tese N)`, `STATUS:` do pipeline, nome de agente/persona, caminho de skill, `{run_id}/vN/`, aviso de "gerado por IA". O escopo é restrito a `squads/*/output/` com nome de peça: `revisao-juridica.md`, `checklist-*.md` e `relatorio.md` **nunca** são bloqueados, porque citam step por dever de ofício — um detector que barra o relatório é desligado no dia seguinte. Um único hook serve às duas IDEs (normaliza o nome do evento). (2) **rubrica de redação no `success_criteria`** dos 6 squads de peça, para o revisor ter voto próprio sobre escrita; (3) **veredito de duas dimensões** no revisor, com `redacao: FAIL` **forçando** `verdict: REJECT`. E o `build.prompt.md` passa a **gerar** as três — sem isso o próximo squad nasceria com o mesmo defeito.
- **Suporte ao Cursor restaurado — a premissa do corte "Claude-only" caiu.** O Cursor foi cortado quando lhe faltavam as três peças de que o orquestrador depende; a IDE passou a ter **subagentes** (`.cursor/agents/*.md`, com janela de contexto isolada — é o que dá o anti-viés do revisor e do verificador de citações), **hooks** (`.cursor/hooks.json`, com bloqueio por `permission: "deny"` **ou** exit 2, mais `failClosed`) e **Skills** (`SKILL.md`). Com isso os dois gates deterministas rodam idênticos nas duas IDEs, e o mesmo arquivo de hook atende às duas.
- **`install-global` instala também no Cursor** (`~/.cursor/`), ao lado do `~/.claude/`: regra global, comando `/criminalsquad`, subagentes e os **dois** hooks. Duas escolhas de projeto, ambas testadas negativamente: a regra do Cursor entra com `alwaysApply: false` (o chefe-roteador se anuncia por relevância, não em toda conversa) e o hook global fica só em `afterFileEdit`, sem `failClosed` — travar a IDE inteira do aluno por um gate do CriminalSquad seria pior que o defeito que ele evita.
- **Skill `resposta-a-acusacao` (CPP 396/396-A/397/401/532)** — a peça de abertura da defesa no rito comum **não existia** numa biblioteca de 521 skills: havia a militar (`jm-resposta-a-acusacao-cppm`) e a eleitoral (`resposta-a-acusacao-crime-eleitoral-rito-355`), e faltava justamente a mais usada. É a lacuna que explica o pior achado da medição limpa do `defesa-criminal-completa`: o run pediu "rejeição da denúncia (CPP 395, III)" numa fase em que a denúncia **já havia sido recebida**.
- **Registro de autoridades ligado como espinha dorsal da curadoria** (`_criminalsquad/core/authorities/*.json`): o mecanismo existia com **um único registro**. Novo `scripts/check-citacoes-em-prompts.mjs` exige que todo `Tema N`/acórdão citado **dentro de prompt** tenha registro de cobertura — o Citation Gate valida a **peça**, não o **prompt**, então número escrito em agente, step ou best-practice nunca passava por gate nenhum e era copiado para a peça como verdade. Saiu de **83 citações não curadas para 0**, e o script passou a **bloquear** por padrão (`--report` para modo leitura).

- **`sync` reindexa sozinho — o baixado aparece na busca do orquestrador na hora.** Fechava o laço prometido pelo S2, mas só na metade: o conteúdo sincronizado chegava ao disco e o `search-skills` (que calcula ao vivo) já o via, mas o `search-acervo` **lê o `acervo/_index.yaml` gerado** — então o material baixado ficava **invisível** até o aluno rodar `indexar-acervo` à mão; e o `skills/_index.yaml` que o catalog-scout/chefe-roteador consultam também ficava velho. Agora, ao final de um `sync` que instalou algo (não em `--check`), o CLI **regenera os dois sumários automaticamente** — `indexSkillsProject(cwd)` para as skills e o próprio `scripts/indexar-acervo.js` para o acervo. É **best-effort**: uma falha na reindexação nunca invalida o que já foi baixado e verificado (o conteúdo está seguro no cache), apenas orienta a reindexar depois. Verificado ponta a ponta pelo CLI real: antes do sync `BUSCA_ACERVO:0`, depois `BUSCA_ACERVO:1` **sem** reindexar à mão. **Endurecimento após revisão adversarial (6 achados, todos confirmados por PoC):** a reindexação **mentia** — imprimia "já aparece na busca" mesmo quando falhava, porque a linha final era incondicional e o guard `err.status == null` classificava crashes reais como benignos; agora `reindexar` rastreia o sucesso de cada passo e só afirma "aparece na busca" quando ambos os índices foram regenerados, caso contrário orienta a rodar `indexar-skills`/`indexar-acervo`. O indexador de acervo **crashava** com `acervo/` ausente (aluno que apagou a pasta; pacote sem semear o acervo local) e vazava o stack cru do Node — agora é robusto (recria a pasta, indexa o que houver) e o `sync` captura o stderr do filho para nunca gritar erro técnico. E ganhou **auto-cura**: o `sync` reindexa não só quando instalou algo agora, mas sempre que os índices estiverem **defasados** em relação ao cache (mtime do `manifest.json` vs. dos `_index.yaml`) — conserta sozinho o aluno que sincronizou com uma versão antiga do CLI ou cuja reindexação falhou antes, mesmo sem novidade da curadoria. Código de saída dedicado (`2`) distingue "classificação inválida no acervo local, índice escrito" de falha real de I/O.
- **Servidor de sync (S3) + rotina de publicação (S4) — o ciclo fecha ponta a ponta.** Com o S2, o conteúdo baixado já aparecia na busca; faltava *de onde* ele vem. **S3 — servidor da curadoria (no ar no Railway, `server/`, Node puro sem dependências, fora do `files[]` do npm):** `GET /v1/catalog` diz a um token o que há de novo e entrega **links de download assinados por HMAC e expiráveis**; `GET /v1/download/:pack/:versao` serve o `.csqpack`; `/admin` é um painel para **emitir, listar e revogar** licenças. O ponto central: **o servidor não assina conteúdo** — os pacotes chegam já assinados pela máquina da curadoria e a chave privada nunca sobe, então servidor comprometido não consegue produzir pacote que o cliente aceite. Entitlement fail-closed: revogada ⇒ 403 sem links, vencida ⇒ 200 **sem** links (o cliente **nunca vira tijolo**, segue no cache). **S4 — `npm run curadoria` / `curadoria:publicar`:** um comando detecta o que mudou em `curadoria/`, roda os **gates**, versiona por data (`AAAA.MM.N`, conteúdo inalterado não gera versão nova), assina e publica; não é cron no servidor **de propósito** — automatiza tudo menos o ato de assinar, que exige a chave privada local. **O gate que não pode falhar:** qualquer arquivo sob `casos/` — em qualquer nível, em qualquer caixa — **bloqueia a publicação inteira** (dado de cliente nunca vira pacote assinado e distribuído); somam-se gates de `SKILL.md` sem frontmatter / `name` divergente / `description` ausente / arquivo vazio. **Endurecimento após revisão adversarial (PoC executável):** `instalacao_id` sem validação inflava `licencas.json` até encher o volume e derrubar o painel → validação de formato + teto de 20 por licença; o freio de rate limit confiava no `x-forwarded-for` do cliente (anulável com um header) → passa a usar o socket / hop confiável (`TRUSTED_PROXY_HOPS`) **e** freio por token; o servidor persistia qualquer `h`, inclusive **CPF em claro** → recusa o que não for `sha256:<hex>` e **re-hasheia com HMAC** de segredo próprio (sha256 puro de CPF é força-bruta de minutos), com rota `esquecer-cpf` (LGPD art. 18); e o servidor deixava de subir mentindo saúde quando um segredo era **curto demais** — agora **recusa iniciar** com config inválida em vez de estourar 500 no primeiro aluno. **Segunda rodada adversarial (mais 12 achados ALTA/MÉDIA confirmados por PoC):** no CLIENTE, defesa contra servidor hostil — o timeout cobria só até os headers (servidor que travava no meio do corpo pendurava o `sync` para sempre) → o mesmo `AbortController` agora vale até o último byte; o teto de 64 MB só era aplicado depois de bufferizar tudo (content-length ausente/mentiroso estourava a memória) → leitura por **streaming** que aborta no primeiro byte além do teto, com teto próprio de 1 MB para o catálogo; **bomba gzip** — o `.csqpack` era descomprimido sem limite antes de qualquer verificação → `maxOutputLength` de 256 MB; **SSRF** — a `url` de download vinha do catálogo (não confiável) sem checagem → recusada se não for da mesma origem `https` do endpoint; catálogo malformado (`packs` não-iterável) vira erro classificado, não `TypeError`. No SERVIDOR: um erro de I/O num download **derrubava o processo inteiro** (`.pipe` sem handler de `error`) → `stream.on('error')` só fecha a conexão; o freio por token colapsava num **balde global `t:`** no `/v1/download` (a turma baixando junta se auto-DoS a 30/min) → freio por IP + por pacote, nunca global; versões antigas de pack **nunca eram removidas** (função de faxina era código morto e ordenava versão por string, `.10` antes de `.9`) → faxina no `publicarPack`, ordenação numérica, mantém as 5 últimas; `licencas.json` corrompido virava `{}` **em silêncio** (toda licença paga sumia) → preserva o arquivo ruim e recusa operar às cegas; `.tmp` de escrita falha vazava no volume → limpeza no boot + `unlink` no erro; trilha **append-only** de emissão/revogação como rede de segurança do cadastro; freio nas rotas `/admin`; e o painel ganhou **revogar-por-token-colado** + filtro (resposta a incidente deixou de exigir caçar a linha numa lista de centenas).
- **Sync de conteúdo da curadoria (S0→S2) — "sincroniza, não serve":** o aluno passa a receber o conteúdo publicado **depois** da instalação (skills novas, jurisprudência, teses) sem reinstalar nada, e **sem que nenhuma consulta saia da máquina** — o pacote é baixado, verificado e gravado local; a busca continua 100% offline. **Integridade fail-closed:** cada pacote traz manifesto com `content_hash` (sha256 dos sha256 por arquivo) assinado em **ed25519**; assinatura inválida, hash divergente ou arquivo faltando ⇒ pacote **recusado e nada é gravado**; a instalação é atômica (monta em `.tmp-` e promove com `rename`), com guarda contra path traversal. **Identidade minimizada (LGPD):** `criminalsquad ativar <CSQ-XXXX-XXXX-XXXX>` grava apenas o **codinome** do titular (marca d'água visível, com `O→0` e `I→1`), a **máscara** e o **hash** do CPF — o CPF em claro nunca é gravado nem transmitido. **Onde mora:** `_criminalsquad/_packs/<pack-id>/{skills,acervo}/`, adicionado a `PROTECTED_PATHS` para que o `update` do npm não apague a curadoria baixada. **S2 — o conteúdo baixado agora APARECE:** os indexadores varrem também o cache (`withPackSkills` em `src/skill-catalog.js`; `scripts/indexar-acervo.js` para o acervo), então `search-skills`, `search-acervo`, `resolve-skills`, `audit-skills` e o `init` enxergam o que veio do sync; cada entrada carrega `fonte_pack: <id>@<versão>` e `verificado_em`, e o acervo sincronizado entra como `VERIFIED_OFFICIAL` (declaração explícita no frontmatter ainda prevalece, inclusive para rebaixar). **Precedência e limites:** o embarcado **sempre vence** (skill de pacote com id colidente é descartada); `acervo/casos/` nunca é indexado, nem dentro de pacote (sigilo); o gate estrutural do `check-skills` não julga conteúdo de pacote — quem responde por ele é a curadoria — mas o **gate de runtime continua valendo**, e por isso os **eval cases e a evidência de forward-run que vêm no pacote** entram na auditoria (sem esse overlay a skill sincronizada reprovaria em `eval_linked` e ficaria bloqueada para sempre — bug pego e travado em teste). Sem nenhum pacote instalado o comportamento é **idêntico ao anterior**. Comandos: `ativar`, `sync [--from <dir>] [--check]`, `status`; ferramenta de publicação `scripts/build-pack.mjs` (chave privada nunca no repositório). **Endurecimento após revisão adversarial (4 lentes × verificadores céticos com PoC executável, 17 achados confirmados):** (1) **CRÍTICA — travessia de caminho por `pack_id`**: `pack_id`/`versao` não fazem parte do material assinado e iam crus para um `join()` seguido de `rmSync` recursivo — um pacote legitimamente assinado, reencaminhado com `pack_id: ../../skills`, **apagava a biblioteca embarcada inteira** e escrevia por cima (PoC reproduzido); agora ambos passam por charset restrito em `verifyPack` **antes de qualquer escrita**, com conferência independente no `installPack` e checagem de que o destino está dentro do cache; (2) **ALTA — defeito do curador virava indisponibilidade do usuário**: um `SKILL.md` sem frontmatter (ou com BOM UTF-8, comum no Windows) estourava `parseScalar(null)` e derrubava `indexar-skills`, `check-skills`, `audit-skills`, `search-skills` **e** `resolve-skills` de uma vez — agora a skill defeituosa é descartada com aviso nomeando o pacote; (3) **ALTA — busca não dizia onde ler a skill sincronizada** (ela não mora em `skills/<id>/`): `search-skills` passa a devolver `skill_path`; (4) **fail-closed no overlay**: só entra no índice o pacote que consta do manifesto de instalação — diretório largado à mão em `_packs/` deixou de ser tratado como curadoria `VERIFIED_OFFICIAL`; (5) **determinismo**: `verificado_em` saiu do `_index.yaml` (hora de parede da instalação faria dois computadores gerarem índices diferentes); (6) precedência entre pacotes unificada (primeiro `pack_id` vence, para skills e evals), `readSkillEvidenceBinding` passou a enxergar os pacotes, classificação de confiança inválida em material sincronizado avisa sem derrubar o `indexar-acervo` do usuário, e as contagens exibidas deixaram de subnotificar o que veio do sync
...
### Changed
- **Caso-ouro deixou de entregar o gabarito junto com o enunciado.** Até 27/07 as armadilhas viviam **dentro** do enunciado: quem executava lia o caso e a resposta no mesmo texto. O gabarito passou a viver em arquivo separado (`*.gabarito.md`) e o produtor lê só o enunciado. A contaminação ficou **medida item a item**, não estimada: no júri, com o gabarito à vista, o run marcou "excesso escusável" como tese doutrinária e **escapou** da armadilha; cego, **caiu nela**. Notas de antes e depois do corte **não se comparam** — o aviso está gravado no topo de cada `_evals/scores.md`. Linhas de base limpas: `negociacao-penal` **82**, `defesa-criminal-completa` **80**, `tribunal-juri` **60**.
- **Regra de limpeza da peça**: o que é andaime de pipeline (status, step, nome de agente, caminho de skill) não pode aparecer no que vai ao protocolo. Antes era pedido em prosa, e a prosa não segurou — agora há o detector determinista que barra antes da gravação.
...
### Fixed
- **Direito positivo errado em 26 pontos das skills do caminho quente** (auditoria da camada substantiva). O pior: `representacao` afirmava que a ameaça em contexto de violência doméstica é de ação penal **incondicionada** — quem seguisse a skill deixaria correr o prazo decadencial de uma ação que **depende** de representação, e decadência não volta atrás. Nenhuma correção inventou jurisprudência: **só texto de lei**.
- **EAOAB art. 17 não trata de conflito de interesses** — a citação estava errada em **83 ocorrências, 54 arquivos**, inclusive no `CLAUDE.md` sempre-carregado e no núcleo. Este é o caso que mostra por que áreas do direito não são independentes: o defeito vivia na **camada transversal do motor** (ética, OAB, LGPD), que **toda** área herda ao forkar.
- **Cinco precedentes citados errado nos prompts, corrigidos ou removidos.** O pior era o `ARE 1.476.455`, um precedente **pró-acusação** oferecido como fundamento de defesa: seguir a orientação levaria o advogado a citar contra o próprio cliente.
- **Os gates deterministas entraram no escopo do lint.** `npm run lint` cobria `src/ bin/ tests/ scripts/` e mais nada — os dois hooks que decidem se uma peça chega ao protocolo ficavam de fora. Um `process` não declarado ou um typo num gate falha **aberto**: o hook quebra, o pipeline segue verde e a peça passa sem verificação nenhuma. Junto: teste de identidade das **três** cópias do `verifica-redacao` (repo + template do Claude Code + template do Cursor), que só o `verifica-citacoes` tinha.
- **`search-acervo` achava por metadado e era cego ao conteúdo** — uma consulta legítima devolvia 0 resultados com o material no disco. Passou a escanear o texto quando o metadado não alcança, pontuando por **co-ocorrência de 2+ termos no mesmo bloco de cabeçalho**, e a devolver o trecho com o número da linha. A consulta que falhava foi de 0 para 3 acertos exatos.
- **Divergência entre a coluna `skills` do CSV e o frontmatter do agente**: 30 divergências em 98 agentes. Novo `scripts/check-party-skills.mjs` detecta e (`--fix`) repara.
- **`verificador-citacoes` era cego**, validação de input assimétrica, 30 comandos `--check` apontando para caminho canônico em vez do real, reset silencioso de memória e `audit-squad` que **passava** em alvo errado — corrigidos (os do motor, também no `legalsquad`).
...
Primeira versão do **CriminalSquad**, focado em advocacia criminal, após a onda de
melhorias de engenharia. Resume as mudanças notáveis desde o início do projeto.
...
- Dashboard: README alinhado ao fluxo real (experimental), remoção de código morto e do campo `gender` órfão; resíduo de nome antigo no lock
