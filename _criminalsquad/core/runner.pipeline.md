...
1b. **Memory format migration** — After loading `memories.md`, check whether it uses the new format by scanning for the `## Estilo de Escrita` section header:
   ...
   - If `OLD_FORMAT` (o arquivo existe e tem conteúdo) → **preserve antes de migrar, e avise**:
     ```bash
     cp squads/{name}/_memory/memories.md "squads/{name}/_memory/memories.anterior.md"
     ```
     e diga ao usuário, em uma linha: *"A memória deste squad estava no formato antigo. Guardei o conteúdo anterior em `_memory/memories.anterior.md` e comecei uma nova."*
     > ⚠️ **Nunca apague memória de squad sem preservar e sem avisar.** O que está ali é o que o profissional ensinou ao squad ao longo das execuções — preferências de redação, proibições, decisões de estilo. Descartar isso em silêncio é perder trabalho do usuário sem que ele saiba, e ele só descobre quando o squad volta a errar o que já tinha aprendido. Os 9 squads distribuídos são entregues com `memories.md` no formato antigo: **a primeira execução de cada um passa por aqui.**
   - If the file is empty or does not exist → nada a preservar; siga direto para (a).
     a. Write `squads/{name}/_memory/memories.md` with the new empty-sections format (o conteúdo antigo **não** é mesclado automaticamente — fica em `memories.anterior.md` para o profissional reaproveitar o que quiser):
        ```markdown
     ...
     b. Check if `squads/{name}/_memory/runs.md` exists:
        ...
        |------|--------|------|--------|-----------|
        ```
   - **Não pause** a execução para a migração — ela não exige decisão. Mas **avise em uma linha** quando houver conteúdo preservado em `memories.anterior.md`: migração que descarta trabalho do usuário não é transparente, é invisível, e são coisas diferentes.

2. Read `squads/{name}/pipeline/pipeline.yaml` for the pipeline definition
3. **Resolve skills com gate de runtime (fail-closed)**:
   a. Monte a união sem duplicatas de: `squad.yaml.skills` + o `skills:` do **frontmatter de cada
      arquivo de agente** (o caminho vem da coluna `path` do `squad-party.csv`). **A autoridade é o
      frontmatter do agente** — é ele que será injetado na execução. A coluna `skills` do CSV é
      resumo legível para humano e **não** é fonte de carregamento: se as duas divergirem, siga o
      frontmatter e reporte a divergência, nunca pare por causa dela. `web_search` e `web_fetch`
      são nativas; mantenha-as na chamada para auditoria, mas elas não exigem `SKILL.md`.
      > Ter duas fontes para a mesma resposta já produziu deadlock: o CSV listava menos skills que
      > o frontmatter, e a regra de Agent Loading manda parar quando uma skill do agente não está
      > no manifesto. Rode `npm run check:party-skills -- --fix` para realinhar o CSV.
   ...
   c. Trate o resultado por código, sem override verbal:
      ...
      - `human-supervision-required` → peça a confirmação de revisão do(a) profissional. **O gate é
        inegociável; a redação dele, não.** Fale a língua do escritório: o que sai daqui é **minuta**,
        e minuta se confere antes de assinar — é assim com estagiário, com colega e com IA
        (Provimento 205/2021 da OAB). Diga o que o(a) profissional confere (o prazo, as citações, a
        tese), não o estágio interno de maturidade da skill.

        **Nunca exponha ao usuário** o vocabulário interno: `contracted`, `quality_status`,
        "contrato estrutural", "validação comportamental", `high_performance_eligible`, ids de
        skill. Para quem lê, isso não soa como cuidado — soa como ferramenta inacabada, e destrói a
        confiança que o gate existe para proteger. Esses termos vivem no log e na auditoria.

        Modelo (adapte ao caso, mantendo uma frase e um pedido):

        > Vou preparar a minuta. Antes de qualquer protocolo, você confere o prazo-fatal e as
        > citações — nada sai daqui automaticamente. Posso seguir?

        Só após o “sim”, rode novamente acrescentando `--supervised`;
...
1. **Pre-Step Input Validation** — MANDATORY. If the step's frontmatter declares an `inputFile`, validate that the input exists before executing the step. Run via Bash tool:
   ...
   ```
   - **Valide o `inputFile` no MESMO caminho em que ele foi gravado.** Aplicar só o Step 1 aqui é o erro que trava o pipeline: o step anterior gravou em `.../{run_id}/vN/arquivo.md` (Steps 1 **e** 2) e a validação procuraria em `.../{run_id}/arquivo.md`, que não existe — do segundo step em diante **toda** validação de input falharia.
   - Os dois casos convivem de propósito, então resolva o caminho assim: aplique o Step 1; procure a **maior** pasta `vN` do grupo; se houver, valide dentro dela; **se não houver, valide o caminho pós-Step 1**. Essa segunda hipótese não é exceção — é o caso normal dos **checkpoints**, que gravam sem versão (ver "Checkpoint files are user input captures", adiante).
     ```bash
     GRUPO="squads/{name}/output/{run_id}/{relative-group}"
     V=$(ls -1 "$GRUPO" 2>/dev/null | grep -E '^v[0-9]+$' | sort -V | tail -1)
     ALVO="$GRUPO/${V:+$V/}{filename}"
...
### Modo smoke (estreia do squad no caso-ouro)

Ativado **somente** pela Fase 5 da criação de squad, e **somente** sobre o
**caso-ouro fictício** (`squads/{name}/_evals/casos/*.md`). É o único gate
comportamental do sistema: sem ele, um squad recém-criado é *plausível*, nunca
*verificado*.

**Isto NÃO é "pular checkpoint".** Pular contradiz o contrato do runner e o do
comando. No modo smoke o checkpoint é **respondido a partir do caso-ouro** — o
gabarito fictício já traz os dados que o usuário forneceria:

1. **`run_id` prefixado `smoke-`** e `state.json` com `modo: smoke`. A saída vai
   para `squads/{name}/output/smoke-{...}/` e **nunca** se mistura ao trabalho
   real do advogado nem contamina o histórico de runs.
2. **Checkpoint de coleta** (pede fatos, datas, anexos do caso): responda com o
   input fictício do caso-ouro. Se o caso-ouro não cobre o que o checkpoint pede,
   **pare** e reporte a lacuna — é defeito do gabarito, não licença para inventar.
3. **Checkpoint de aprovação de rascunho** (aprovar a minuta?): aprovado
   automaticamente, registrando no `state.json` que a aprovação foi do smoke, não
   humana. É seguro porque o artefato é fictício e não sai da máquina.
4. **Checkpoint que antecede ATO IRREVERSÍVEL** — protocolar, enviar e-mail,
   publicar, assinar: **PARE ALI**. Auto-aprovar ato irreversível é **proibido**,
   inclusive em teste; o smoke termina nesse ponto e o que foi produzido até ali é
   o que vai para avaliação. Nada é protocolado, enviado ou publicado num teste.
5. **Todo o resto continua valendo, sem exceção:** Citation Gate, Post-Step Output
   Validation, Veto Enforcement, `on_reject`/`max_review_cycles` e versionamento.
   O smoke exercita o squad de verdade — um squad que só passa com os gates
   desligados não passou.
6. **O caso-ouro É o insumo — não bloqueie por anexo que o smoke não tem.** No
   primeiro smoke dos 9 squads, quatro reprovaram fechando `REJECT` porque
   faltavam PDFs, autos ou pasta de caso — arquivos que **não existem por
   desenho**, já que o caso é fictício e `acervo/casos/` é gitignored. Isso é
   **sobre-bloqueio**: confundir limitação do ambiente com defeito do caso.
   A distinção é precisa e **não afrouxa o fail-closed**:
   - **etapa obrigatória não executada** (pesquisa não rodou, cálculo não foi
     feito, Citation Gate não conferido) ⇒ `REJECT`, como sempre;
   - **anexo físico ausente porque o caso é fictício** ⇒ **não é bloqueio**: a
     narrativa do caso-ouro substitui o anexo. Trate o que ela descreve como o
     conteúdo daquele documento e siga, registrando a premissa.
   Se o caso-ouro realmente não traz o que o step precisa, aí sim **pare** — mas
   reporte como **lacuna do gabarito**, não como falha do squad (item 2).

Ao final, o output do smoke é entregue ao subagente `avaliador-squad` (contexto
isolado — quem redigiu não se avalia), e a nota é registrada em
`squads/{name}/_evals/scores.md`.
...
3. **Loop gerador→verificador (teto: `max_citation_cycles`, default 3).** Se o veredito for Revisão humana necessária, devolva ao step de redação **apenas** as citações problemáticas para correção/remoção e **reverifique**. Pare em APROVADO ou ao atingir o teto — neste caso **escale ao usuário** com a lista de pendências; **não** finalize.
...
- Never continue past a checkpoint without user input. **Única exceção — "Modo
  smoke" (acima):** na estreia do squad sobre o caso-ouro fictício, o checkpoint é
  respondido *pelo caso-ouro*, nunca ignorado, e o smoke **para** diante de
  qualquer ato irreversível. Fora do modo smoke, não há exceção.
