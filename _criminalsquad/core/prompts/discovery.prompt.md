...
Em seguida, reduza o objetivo a termos de capability sem dados do caso e faça uma **busca em leque**: derive **3 a 6 termos-âncora** — por **lei especial**, **tipo penal**, **peça**, **fase** e **tese/instituto** — e rode `npx criminalsquad search-skills --query "<termo-âncora>" --limit 8 --json` **uma vez por termo**, unindo os resultados. Uma consulta só não recupera as sub-capacidades irmãs (medido: 3 de 5 skills em tributário, 6 de 7 em protetivas; com o leque, 5/5 e 7/7) — e recall baixo faz o Arquiteto declarar lacuna e criar skill **duplicada**. Use somente a shortlist unida; não leia `skills/_index.yaml` por inteiro. Em trabalho de evolução, `--include-preview` revela fontes de teste. Consulte `skills/_execucao-penal-v3-integration.yaml` apenas por busca direcionada quando a shortlist ou o domínio exigir canonicalização das `ep-*`.
...
**If domain = `legal`:**
1. **Em que FASE do processo criminal este squad atua?** É a pergunta que mais
   define o desenho — antes do formato do entregável. Múltipla escolha:
   **estratégia/consultivo** (antes de existir processo: teoria do caso, risco,
   decisão de agir) / **investigação** (inquérito, PIC, defesa no IP, trancamento) /
   **cautelares** (prisão, medidas protetivas, assecuratórias — relógio próprio) /
   **1º grau** (resposta à acusação, instrução, memoriais) / **tribunal do júri**
   (1ª fase: pronúncia — 2ª fase: plenário) / **recursos ordinários** (apelação,
   RESE, agravo, embargos) / **tribunais superiores** (REsp, RE, AREsp, HC/RHC no
   STJ e no STF, repercussão geral) / **execução penal** / **justiça negocial**
   (ANPP, colaboração, transação) / **outra**.
   > A fase mune a pergunta seguinte. Ex.: em **tributário** só se pergunta a
   > constituição definitiva do crédito se a fase for investigação/1º grau; em
   > **protetiva** a fase cautelar impõe o prazo de 48h; em **tribunais
   > superiores** o que decide o desenho é o prequestionamento e a admissibilidade,
   > não a instrução.
2. Qual o **entregável** ao fim do pipeline? (peça protocolável / parecer ou
   memorando interno / relatório de análise / rotina de monitoramento / conteúdo).
   Fase e entregável são coisas diferentes: a mesma fase produz peça **ou** parecer.
3. Qual o **polo de atuação** deste squad? (defesa / acusação-querelante / assistente de acusação) — herde do `company.md` se já definido e apenas confirme.
4. Há **prazo/tempestividade** crítico (ex.: 10 dias do art. 396 CPP, prazo recursal)? E qual o **nicho penal** (drogas, júri, violência doméstica, econômico/tributário, crimes contra a honra, geral)?
> Sempre: conflito de interesses (art. 15, § 6º, EAOAB + CED 19-21) na triagem; toda peça passa pelo Citation Gate (subagente `verificador-citacoes`) + revisão humana obrigatória.
...
context:
  ...
  output_format: "{answer from Step 3}"
  # For legal squads:
  # `fase` é o campo que mais determina o desenho — o Design e o Build leem dele
  # para escolher papéis, rito e a estrutura da peça. Sem ele, todo squad nasce
  # com o formato de resposta à acusação, que não serve a HC, protetiva,
  # execução nem memorial ao STF.
  fase: "{estrategia | investigacao | cautelares | primeiro-grau | juri-1a-fase | juri-plenario | recursos-ordinarios | tribunais-superiores | execucao-penal | justica-negocial | outra}"
  entregavel: "{peca-protocolavel | parecer-interno | relatorio-analise | rotina-monitoramento | conteudo}"
