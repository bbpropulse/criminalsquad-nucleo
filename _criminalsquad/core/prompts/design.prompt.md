...
2. **Mapeie cada necessidade do squad a uma família de capacidade** (reaproveite, não reinvente):
   ...
   - **Leitura multimodal dos autos** — as skills `leitura-*` (still de CFTV, laudo pericial em imagem, autenticidade de print, EXIF, croqui), `ocr-autos-pdf` (PDF escaneado) e, para **áudio/vídeo** (audiência de custódia, depoimento gravado, CFTV, interceptação), a skill `captura-midia-av` / o comando `npx criminalsquad captura` — transcrição **local** por padrão, para preservar o sigilo. Mídia de audiência é insumo comum no criminal e some da busca quando a consulta é só por "documento".
   - **Cálculo determinístico** — `calculadora-dosimetria` (pena), `calculadora-prescricao` (prescrição), `calculadora-tempestividade` (**prazo recursal**, CPP 798, contado da intimação), `calculadora-pena-multa`. **Se o passo envolve pena, prescrição ou prazo, o step INVOCA a calculadora — nunca deixe o agente "calcular de cabeça".** Antes de atribuir, **confira o escopo declarado no `SKILL.md` contra a família do prazo**: não há motor para prazo de prisão/custódia, conclusão de IP (CPP 10; art. 51 e 54 da Lei 11.343) nem duração de cautelar — esses correm **da prisão**, e o motor recursal prorroga o vencimento **contra o preso**. Nesses casos registre GAP no design e marque `[A CONFERIR]` no step; **jamais reaproveite motor de outra família porque "conta dias"**.
   ...
   - **Extinção da punibilidade / válvulas de saída** — prescrição, decadência, perempção, retratação, pagamento do tributo (extinção nos crimes tributários), parcelamento (suspensão), morte do agente, *abolitio criminis*. **Sempre confira esta família antes de montar defesa de mérito:** a saída mais barata para o cliente costuma estar aqui, e ela some da busca quando a consulta é só pelo nome do crime.
   - **Compliance e investigação interna corporativa** — apuração interna, whistleblower, acordo de leniência, responsabilidade da PJ, prevenção. Família própria: não se acha buscando pelo tipo penal.
   - **Integrações** (DJEN, e-mail, agenda, publicação em redes, assinatura) — precisam de setup (env/MCP).
3. **Como cada tipo entra no squad:**
   - Skills de **peça/análise/cálculo/estratégia**: entram no STEP **e** no `skills:` do AGENTE que a usa — os dois, sempre. **Não precisam de instalação** — já vêm com o CriminalSquad. Registre-as no design.yaml.
   ...
   > **Por que nos dois lugares (não é redundância).** O gate de runtime monta seu manifesto da união `squad.yaml.skills` + `skills:` de cada agente do `squad-party.csv` — **a prosa dos steps não entra nessa conta** (ver `runner.pipeline.md`, "Resolve skills com gate de runtime"). Skill nomeada só no step é carregada de verdade e **nunca passa** pelo gate de lifecycle/maturidade/evidência: é assim que uma `quarantined` entraria em produção. Medido antes desta regra existir: 10 skills nessa situação nos squads embarcados.
   >
   > Divisão de papéis: o **step** diz *quando* carregar e *para quê*; o **agente** declara *o que a persona carrega* — e é o que submete a skill ao gate. Confira com `npx criminalsquad audit-squad -- <squad>`: qualquer item sob `[DEFEITO] skill fora do manifesto` é bypass do gate, não recomendação.
...
5. Registre no design.yaml: quais skills os steps carregam (núcleo), quais integrações foram instaladas e a decisão de lifecycle/canonicalização (selecionadas, recusadas e fallback de qualquer pilot).
   ...
   > **`lifecycle` é POR SKILL, lido do índice — nunca atribuído a um grupo pelo prefixo.** Medido: um design recusou o grupo `execucao-*` registrando `lifecycle: "pilot"`, quando das 36 skills com esse prefixo **20 são `active` e 16 são `pilot`**. A decisão de recusar até estava certa pelo outro fundamento (domínio diferente), mas o atributo lavrado no registro contradizia 20 dos 36 membros — e é esse registro que a próxima pessoa vai ler como fato. Se o motivo real é o domínio, escreva o domínio. Só escreva `lifecycle: pilot` sobre uma skill cujo `lifecycle` você **leu** em `skills/_index.yaml`.
...
- For legal/criminal squads, os **invariantes** valem sempre, em qualquer fase: pesquisa = step subagente que consulta o `acervo/` antes da web; redação = step que **carrega a skill da peça** (a skill define a estrutura, não este prompt); revisão = subagente isolado com preclusões/nulidades e o gate ético (best-practice `etica-oab-sigilo`).
- **Espelhar a ESTRUTURA de um squad-exemplo (`defesa-criminal-completa`, `recurso-criminal`…) só quando a `fase` do `discovery.yaml` for a mesma.** Os 9 squads que vêm no pacote são **exemplos de fases específicas**, não gabarito universal. Fase diferente = desenho diferente:
  - **estratégia/consultivo** — sem peça protocolável; termina em parecer e decisão de agir. Não force pipeline de redação.
  - **investigação** — o eixo é acesso aos autos, diligências e trancamento; a "peça" pode ser requerimento ou HC.
  - **cautelares/protetivas** — o relógio (48h) manda no pipeline; steps curtos, checkpoint cedo.
  - **júri** — 1ª fase (pronúncia) e plenário são pipelines distintos; não os funda num só.
  - **tribunais superiores** — o eixo é **admissibilidade** (prequestionamento, repercussão geral, súmulas 7/83 STJ e 279/280 STF), não instrução. Um squad de REsp/RE que copie a estrutura de 1º grau nasce errado.
  - **execução penal** — cálculo determinístico e marco temporal são o centro; ver `execucao-penal-alta-performance`.
  Na dúvida entre espelhar e desenhar do zero, **desenhe do zero a partir da fase** e reuse só os invariantes acima.
...
- **O acervo NÃO é "material verificado" — `confianca` é campo, não adjetivo.** Medido: um squad gerado escreveu que o acervo é "material já curado e **VERIFICADO** pelo escritório", enquanto os dois arquivos em que ele se apoiava voltavam de `search-acervo` com `confianca: DISCOVERY_ONLY` e `verified: false`. O índice inteiro tem **1 VERIFIED_OFFICIAL entre 46 entradas** (as outras 45 são DISCOVERY_ONLY) — a distinção é operativa, não decorativa. Leia o campo `confianca` de cada resultado e trate `DISCOVERY_ONLY` pelo que é: **pista de onde procurar, não fonte que satisfaz o Citation Gate**. Nunca escreva no step que o acervo é verificado.
