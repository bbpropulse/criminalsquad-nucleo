...
4. **Precisão de citação.** Toda referência deve trazer **número do processo, relator,
   órgão julgador, data de julgamento** e, quando houver, o **número do Tema** (`Tema NNN/STF`,
   `Tema NNN/STJ`). Citação sem esses dados é inverificável e fragiliza a peça.
...
- **Citar repetitivo apenas afetado como se a tese estivesse fixada.** Tema afetado mas
  ...
  fundamentar mérito.
- **Esquecer o número do Tema (`Tema NNN/STF`, `Tema NNN/STJ`).** Sem o número, a conferência
