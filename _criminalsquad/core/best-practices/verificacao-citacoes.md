...
**Quando a fonte não abre — falha de acesso não é permissão para afirmar.** Portal do tribunal fora do ar, timeout, erro de rede: o item **não** vira `verificada` "porque a citação parece certa". Ele é `não verificada` como qualquer outro — a citação **sai da peça** e volta quando a fonte for conferida de fato. No manifesto do Citation Gate, o status honesto é **`acesso_falhou`**, com `source_url` e `consulted_at` guardando o registro da tentativa; declarar isso passa no gate, mas **manter a citação no texto com esse status bloqueia**. Uma peça com citação material precisa de ao menos uma citação `verificada`: um manifesto inteiro de `acesso_falhou` não atesta nada.
