...
```json
{
  ...
  "resultado_principal": {
    ...
    "cenarios": [
      {
        "nome": "oficial|conservador|defensivo|risco",
        "premissa": "",
        "resultado": "",
        "impacto_em_dias": "",
        "risco": ""
...
- Há título executivo e guia suficientes para auditar pena?
- Há cálculo oficial ou atestado para comparar?
- A data-base tem origem documentada?
- Há remição, detração, falta grave ou nova condenação que altere marcos?
- A conclusão depende de fração controvertida?
...
- Não entregar data única quando houver tese controvertida; entregar cenários.
- Separar remição homologada de remição apenas documentada/pendente.
- Toda fração deve vir acompanhada da premissa jurídica que a sustenta.
- Quando houver nova condenação, indicar se o cálculo antes/depois da unificação muda o marco.
...
O cálculo oficial ignorou 90 dias de remição homologada e aplicou fração sem explicitar natureza do crime; a saída deve produzir cenário oficial, cenário defensivo, impacto em dias e providência de impugnação.
...
```json
{
  ...
  "resultado_principal": {
    "campos_especializados": ["pena_por_condenacao", "memoria_matematica", "data_base_oficial", "data_base_defensiva", "detracoes", "remicoes", "faltas", "progressao", "livramento", "termino", "divergencias_em_dias"]
  },
  "achados_criticos": [
  ...
  "proximas_skills_sugeridas": [
    {"skill": "ep-revisor-calculo", "motivo": "continuidade técnica recomendada"},
    {"skill": "ep-beneficios-vencidos-detector", "motivo": "continuidade técnica recomendada"},
    {"skill": "ep-peca-impugnacao-calculo", "motivo": "continuidade técnica recomendada"}
  ],
  "revisao_humana_obrigatoria": true
}
```
