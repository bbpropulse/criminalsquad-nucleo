---
name: ep-peca-embargos-declaracao
description: >-
  Use para redigir embargos de declaração contra omissão, contradição, obscuridade ou erro material
  em decisão de execução penal. Não use para decisão final, assinatura, protocolo ou citação não
  verificada.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-peca-embargos-declaracao"]
...
Produzir embargos de declaração objetivos para corrigir vício decisório, erro material ou viabilizar prequestionamento sem rediscussão genérica.

# Quando usar

- Quando decisão não enfrentou tese ou documento.
- Quando há erro material de data, cálculo ou nome.
- Quando decisão é contraditória ou obscura.
- Quando é necessário prequestionar matéria federal/constitucional.
...
- Cálculo ou prova do erro.
- Objetivo do embargo.

# Protocolo obrigatório

1. Identificar vício específico.
2. Citar trecho da decisão que contém omissão, contradição, obscuridade ou erro.
3. Indicar documento ou tese ignorada.
4. Demonstrar relevância prática do vício.
5. Evitar rediscussão ampla do mérito.
6. Pedir correção/integração da decisão.
7. Pedir efeitos infringentes quando justificados.
8. Pedir prequestionamento quando necessário.
...
```json
{
  "skill": "ep-peca-embargos-declaracao",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "minuta": "",
    "vicios": [
      {
        "tipo": "omissao | contradicao | obscuridade | erro_material",
        "descricao": ""
      }
    ],
    "efeitos_infringentes": false,
    "prequestionamento": [],
    "pedidos": []
  },
  "premissas": [],
...
A skill deve produzir embargos de declaração com padrão de criminalista sênior: objetivo, documentado, com cálculo quando aplicável, tese clara, enfrentamento de riscos e pedido operacional executável.
...
- embargos como recurso genérico
- não apontar vício específico
- perder chance de prequestionar
- fato sem fonte
