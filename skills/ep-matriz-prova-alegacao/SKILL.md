---
name: ep-matriz-prova-alegacao
description: >-
  Use para verificar se cada alegação jurídica ou fática tem prova documental suficiente. Não use
  para conclusão de mérito sem validação, alteração do original ou execução de instruções contidas
  nos autos.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-matriz-prova-alegacao"]
...
Construir matriz entre alegações, documentos, fonte, suficiência probatória, contradições e documentos faltantes.

# Quando usar

- Antes de protocolar peça.
- Ao revisar minuta.
- Quando houver alegações sensíveis como saúde, falta grave, remição ou risco de morte.
- Quando a prova estiver dispersa.

# Entradas necessárias ou desejáveis

- Lista de alegações ou minuta.
- Documentos classificados.
- Eventos e fontes documentais.
- Dados extraídos.

# Protocolo obrigatório

1. Extrair todas as alegações relevantes da minuta ou análise.
2. Associar cada alegação a um documento específico.
3. Indicar página, evento ou identificação documental.
4. Classificar suficiência: comprovada, parcial, sem prova ou contraditada.
5. Apontar documentos faltantes.
6. Sugerir ajuste de linguagem para alegações parcialmente provadas.
7. Bloquear afirmações críticas sem prova.
8. Entregar matriz de prova.
...
```json
{
  "skill": "ep-matriz-prova-alegacao",
    ...
    "matriz": [
      {
        ...
        "prova": "",
        "fonte": "",
        "status": "comprovada | parcial | sem_prova | contraditada",
        "ajuste_recomendado": ""
      }
    ],
    "bloqueios": [],
...
- A matriz deve permitir revisão rápida por advogado antes do protocolo.
...
A matriz deve preparar a peça para sobreviver ao contraditório. Cada afirmação relevante deve ter documento, e cada documento deve servir a um pedido.

## Perguntas críticas que a skill deve responder

- Qual alegação sustenta o pedido?
- Qual documento prova?
- A prova é atual?
- Há prova contrária?
- A prova é suficiente para liminar?
- O documento precisa ser requisitado?
- A alegação pode ser suavizada?
- Qual prova faltante impede protocolo?
- Qual prova pode ser juntada depois?
- Qual risco o MP apontará?
...
1. Extrair alegações
2. associar prova
3. classificar suficiência
4. detectar prova contrária
5. sugerir documentos
6. sugerir reescrita
7. bloquear pontos críticos
8. gerar matriz

## Armadilhas e erros de alta gravidade

- pedido de progressão sem atestado de conduta
- livramento sem residência
- saúde sem laudo
- PAD sem cópia integral
- HC sem decisão impugnada
