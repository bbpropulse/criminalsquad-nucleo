---
name: ep-extrator-dados-executorios
description: >-
  Use para extrair dados estruturados essenciais da execução penal: pena, crimes, datas, regime,
  faltas, remições e benefícios. Não use para conclusão de mérito sem validação, alteração do
...
metadata:
  ...
  eval_case_ids: ["csq-v5-ep-extrator-dados-executorios"]
...
Converter documentos de execução penal em dados estruturados auditáveis para cálculo, diagnóstico, peças e revisão.

# Quando usar

- Depois de classificar documentos.
- Antes da auditoria de cálculo.
- Ao montar banco de dados do caso.
- Quando houver necessidade de alimentar dashboard ou relatório.

# Entradas necessárias ou desejáveis

- Sentença, acórdão, guia, cálculo, atestado, certidões, PADs, decisões e documentos de remição.
- Metadados documentais.

# Protocolo obrigatório

1. Extrair dados de identificação do processo.
2. Extrair todas as condenações e penas por condenação.
3. Extrair datas relevantes: fato, prisão, sentença, trânsito, guia, início da execução e data-base.
4. Extrair regime inicial, atual e pretendido, se houver.
5. Classificar natureza dos crimes sem inferir além dos documentos.
6. Extrair remições homologadas e pendentes.
7. Extrair faltas graves, PADs e decisões de homologação.
8. Extrair benefícios anteriores e decisões pendentes.
9. Marcar dados conflitantes e ausentes.
10. Entregar JSON estruturado.
...
```json
{
  "skill": "ep-extrator-dados-executorios",
...
- Cada dado extraído deve ter fonte documental sempre que possível.
...
A extração deve criar base única de verdade do caso. Dados extraídos alimentam cálculo, peças e dashboard, por isso a skill deve ser conservadora e auditável.

## Perguntas críticas que a skill deve responder

- Quantas condenações existem?
- Qual pena por condenação?
- Qual artigo e data do fato?
- Há trânsito para acusação/defesa?
- Qual regime inicial e atual?
- Há detração?
- Há remição?
- Há falta grave?
- Há nova condenação?
- Qual é o cálculo mais atual?
...
- decisões de benefício
- certificados

## Protocolo ampliado de alta performance

1. Extrair identificação
2. extrair condenações
3. extrair datas
4. extrair regime
5. extrair natureza do crime
6. extrair remições
7. extrair faltas
8. extrair benefícios
9. marcar conflitos
10. gerar JSON

## Armadilhas e erros de alta gravidade

- pena total sem separar condenações
- crime classificado sem sentença
- data do fato confundida com data da sentença
- regime atual inferido por relato
