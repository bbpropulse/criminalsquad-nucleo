# CriminalSquad Skill Format Reference

## SKILL.md Structure

Every CriminalSquad skill consists of a `SKILL.md` file with YAML frontmatter and a Markdown body.

## Contrato operacional v5 (obrigatório)

Toda skill do CriminalSquad cumpre o **contrato operacional v5**: não basta `name`/`type`/`version` — o contrato acrescenta perfil de qualidade, risco, guards, política de atualidade e um eval vinculado.

> **Cumprir o contrato é estrutural — não é desempenho comprovado.** Toda skill nasce `contracted`. `high_performance_eligible` é **computado** (nunca declarado) em `src/skill-quality.js` e só fica `true` com `quality_status: verified`/`certified`, que exigem **evidência comportamental**: forward-run persistido + baseline com/sem a skill provando melhoria + revisores independentes, amarrados aos bytes exatos do `SKILL.md`. Declarar `verified` sem esse envelope é **hard fail** no `audit-skills`.

Leia `_criminalsquad/core/best-practices/skills-alta-performance.md` (princípios, contrato mínimo, hard fails) antes de criar.

**Você não escreve o contrato à mão — um pipeline determinístico o aplica.** O fluxo:

1. **Autore o `SKILL.md`** com um frontmatter inicial mínimo e um corpo denso e específico:
   ```yaml
   ---
   name: minha-skill
   description: >-
     Use ao {trabalho + gatilhos}. Não use para {fronteira de não uso}.
   metadata:
     type: "prompt"          # prompt | mcp | script | hybrid
     version: "1.0.0"
     categories: [dominio, subdominio]
     lifecycle: "active"
   ---
   ```
2. **Aplique o contrato:** `npx criminalsquad contract-skills`. Isso normaliza o frontmatter para v5 (adiciona, sob `metadata:`, `schema_version: "5"`, `quality_profile`, `contract_version`, `quality_status`, `risk_level`, `delivery_type`, `freshness_policy`, `positive_triggers`/`negative_triggers`/`guard_triggers` e `eval_case_ids`), injeta o bloco `<!-- CRIMINALSQUAD:HP-CONTRACT -->` logo após o `# título`, gera `references/high-performance-contract.md` e `agents/openai.yaml`, registra o eval `csq-v5-{name}` em `skills/_evals/catalog-v5.json` e regenera `skills/_index.yaml`. É idempotente.
3. **Valide:** `npx criminalsquad audit-skills` (contrato/guards/risco, sem hard fail) e `npx criminalsquad check-skills` (índice fresco, grafo válido). Corrija e repita até verde.

A skill nasce `quality_status: contracted` — contrato estrutural, **não** desempenho comprovado. A evidência comportamental (forward-run + baseline) vem do loop de eval descrito no `SKILL.md` do skill-creator e promove a skill para `verified`/`certified`.

Para skills `mcp`/`script`/`hybrid`, os blocos `mcp:`/`script:` (campos abaixo) passam a viver **sob `metadata:`** após o contrato. As tabelas a seguir descrevem quais campos existem por tipo; o pipeline os posiciona corretamente.
