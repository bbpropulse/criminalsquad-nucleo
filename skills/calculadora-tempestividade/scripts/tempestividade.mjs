...
//
// Regra penal: prazos CONTÍNUOS. O caput do art. 798 diz que não se interrompem
// por "férias, domingo ou dia feriado" — **sábado não está no texto**. Não está
// porque não precisa: contagem contínua não para em dia nenhum. Escrever
// "sábado" dentro da citação do caput é alterar texto de lei, e este motor já
// fez isso — o smoke-run de 27/07/2026 pegou.
//
// §1: exclui-se o dia do começo e inclui-se o do vencimento.
// §3: o prazo que terminar em "domingo ou dia feriado" prorroga-se para o dia
//     útil imediato. **Sábado também não está aqui.** A prorrogação quando o
//     vencimento cai num sábado é prática ancorada na AUSÊNCIA DE EXPEDIENTE
//     FORENSE, não no § 3º — e por isso sai como aviso de premissa, nunca como
...
function calcularTempestividade(input) {
  ...
  // Vencimento em dia não útil → prorroga para o próximo útil.
  // ATENÇÃO AO FUNDAMENTO: o § 3º nomeia "domingo ou dia feriado". Quando o
  // vencimento cai num SÁBADO, a prorrogação não vem do § 3º — vem da ausência
  // de expediente forense. O resultado é o mesmo; a citação, não. Invocar o § 3º
  // para sábado é citar dispositivo que não diz aquilo, e numa peça isso é o
  // tipo de erro que a parte contrária usa.
  const prorrogado = !ehUtil(limite, feriados);
  const venceuNoSabado = limite.getUTCDay() === 6;
  if (prorrogado) limite = proximoUtil(limite, feriados);
  if (prorrogado && venceuNoSabado) {
    avisos.push(
      `Vencimento caiu num SÁBADO (${limiteBruto}) e foi prorrogado para ${iso(limite)}. `
      + 'FUNDAMENTO: o § 3º do art. 798 do CPP nomeia "domingo ou dia feriado" — sábado NÃO está no dispositivo. '
      + 'A prorrogação aqui se apoia na ausência de expediente forense no sábado, e isso é PREMISSA a confirmar '
      + 'na regra do tribunal (portaria/regimento). Não cite o § 3º como fundamento da prorrogação por sábado.',
    );
  }
