...
- **VIII** — decretar a prescrição ou julgar, **por outro modo**, extinta a punibilidade — **por decisão interlocutória** (se no bojo da sentença → apelação, art. 593, §4º).
...
Inciso **expressamente revogado** (não confundir com os tacitamente revogados abaixo):
- **VI** — revogado pela **Lei 11.689/2008**. Era o RESE contra a antiga absolvição sumária do júri (art. 411 na redação anterior); hoje a absolvição sumária é o art. 415 e o recurso é a **apelação do art. 416**.

> ⚠️ **Cuidado com o deslocamento de um inciso.** Como o VI foi revogado, é comum a lista "escorregar": quebra/perda da fiança é o **VII** (não o VI) e a extinção da punibilidade é o **VIII** (não o VII). Interpor RESE indicando o inciso errado entrega ao MP a preliminar mais barata que existe — **confira a numeração na fonte oficial antes de peticionar**.
...
- **Pronúncia = 581, IV → despronúncia (414); desclassificação = 581, II.**
  ⚠️ **A despronúncia depende da PROVA, não do rito.** O art. 414 exige que o juiz
  não se convença da materialidade **ou** de indício suficiente de autoria. Se a
  **autoria é incontroversa** — o réu admitiu o fato e alega excludente —, pedir
  impronúncia é tecnicamente incoerente com os autos e enfraquece o recurso: a via
  ali é **absolvição sumária** (415, IV), **desclassificação** (419) ou **decote**
  de qualificadora. Confira a prova antes de escolher o pedido; não deduza a tese
  do tipo de decisão atacada.
