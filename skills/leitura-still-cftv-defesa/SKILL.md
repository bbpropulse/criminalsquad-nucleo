...
- **Etapa anterior (quando os autos trazem o VÍDEO, não o still):** use a skill `captura-midia-av` para extrair os frames com **tempo carimbado** (`npx criminalsquad captura "<arquivo>" --sigiloso`; para CFTV/bodycam, `--every SEG` varre segundo a segundo sem perder o momento decisivo) e só então leia cada frame por aqui. Extrair o próprio frame evita depender do recorte que a acusação escolheu juntar.
