---
name: ep-extincao-pena-alvara
description: >-
  Use para verificar término de pena, extinção, alvará, mandados pendentes e cumprimento efetivo de
  soltura. Não use para conclusão definitiva sem autos suficientes, fonte atual ou revisão
  profissional.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-extincao-pena-alvara"]
...
Auditar se a pena já terminou ou se decisão de soltura/extinção precisa ser expedida, corrigida ou cumprida imediatamente.

# Quando usar

- Quando o cálculo aponta término próximo ou vencido.
- Quando há decisão favorável não cumprida.
- Quando alvará não foi expedido ou cumprido.
- Quando pode haver outro mandado impedindo soltura.

# Entradas necessárias ou desejáveis

- Cálculo atualizado.
- Atestado de pena.
- Decisão de extinção, livramento ou alvará.
- BNMP/mandados, se disponíveis.
- Informações da unidade prisional.
- Linha do tempo.

# Protocolo obrigatório

1. Verificar pena total, remição, detração e término.
2. Confirmar se há decisão de extinção ou soltura.
3. Verificar expedição do alvará.
4. Verificar cumprimento do alvará.
5. Identificar inconsistências de nome, processo, CPF, mandados ou guias.
6. Verificar existência de outros mandados ou processos impeditivos.
7. Identificar excesso de execução por permanência indevida.
8. Indicar petição urgente e providências operacionais.
9. Entregar checklist de soltura.
...
```json
{
  "skill": "ep-extincao-pena-alvara",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "extincao_alvara": {
      "termino_pena": "",
      "pena_extinta": "sim | nao | depende",
      "alvara_expedido": "sim | nao | nao_informado",
      "alvara_cumprido": "sim | nao | nao_informado",
      "impedimentos_possiveis": [],
      "providencia_urgente": "",
...
- A saída deve ser operacional e priorizar liberdade se houver excesso.
...
A skill deve impedir prisão além do devido. Verifica término, extinção, alvará, mandados e cumprimento operacional.

## Perguntas críticas que a skill deve responder

- A pena terminou?
- Há remição que antecipa término?
- Há livramento com período de prova encerrado?
- Há alvará expedido?
- Foi cumprido?
- Existe outro mandado?
- Há erro cadastral?
- Há pendência de multa/restritiva?
- Há BNMP a conferir?
- Qual petição urgente?

## Documentos e dados específicos a procurar

- cálculo
- atestado
- decisão de extinção/alvará
- mandados
- certidão carcerária
- documentos de remição
- BNMP se disponível

## Protocolo ampliado de alta performance

1. Calcular término
2. verificar decisões
3. verificar alvará
4. verificar cumprimento
5. checar mandados pendentes
6. detectar erro cadastral
7. indicar providência urgente

## Armadilhas e erros de alta gravidade

- alvará não cumprido
- término vencido
- outro mandado não verificado
- erro de nome/CPF
- decisão favorável parada
