...
- **Sigilo e LGPD:** autos e inquéritos frequentemente correm em segredo de
  justiça e contêm dados sensíveis. Faça o **OCR localmente** (Tesseract/ferramenta
  do ambiente) — assim o **PDF não sai da máquina**. O **texto extraído**, esse sim,
  entra no contexto do agente de IA quando você pede a leitura ou a análise (como
  qualquer documento que você entrega a ele); por isso extraia só o que for
  necessário ao trabalho, não repasse os autos a terceiros e armazene apenas em
  `acervo/casos/{numero-processo}/` (gitignored) ou na pasta de saída do squad.
