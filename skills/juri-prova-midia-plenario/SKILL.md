...
- **Prepare a mídia antes do plenário:** rode `captura-midia-av` sobre a gravação (`npx criminalsquad captura "<arquivo>" --sigiloso`) para ter **hash, duração, transcrição com timestamp e frames** — é o que permite escolher o trecho exato a exibir, indicar a minutagem quando o juízo ou o jurado pedir a folha (art. 480) e detectar corte/edição antes que a mídia chegue ao Conselho. Depois, examine o material com `leitura-still-cftv-defesa` (frame) e `leitura-video-forense-deepfake-adulteracao` (vídeo).
