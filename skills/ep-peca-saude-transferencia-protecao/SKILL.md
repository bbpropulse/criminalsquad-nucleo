---
name: ep-peca-saude-transferencia-protecao
description: >-
  Use para redigir pedido urgente de saúde, transferência, proteção, apuração de tortura/maus-tratos
  ou providência carcerária. Não use para decisão final, assinatura, protocolo ou citação não
  verificada.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-peca-saude-transferencia-protecao"]
...
Produzir minuta urgente para proteger direito concreto da pessoa presa, com prova, risco, pedido operacional e prazo de cumprimento.

# Quando usar

- Quando há risco de saúde.
- Quando há ameaça, tortura, maus-tratos ou necessidade de proteção.
- Quando é necessária transferência.
- Quando visitas, higiene, alimentação, banho de sol ou atendimento são negados.

# Entradas necessárias ou desejáveis

- Relato e documentos comprobatórios.
- Laudos, receitas, fotos, declarações ou ofícios.
- Pedidos administrativos e respostas.
- Dados da unidade.
- Indicação de urgência e risco.

# Protocolo obrigatório

1. Identificar fato urgente e risco concreto.
2. Separar prova disponível e prova a requisitar.
3. Apresentar linha do tempo curta da omissão.
4. Fundamentar a responsabilidade do juízo da execução no controle da pena.
5. Formular pedido operacional com prazo.
6. Pedir ofícios, requisição de prontuário, inspeção ou comunicação a órgãos quando necessário.
7. Pedir sigilo quando a exposição aumentar risco.
8. Formular pedido subsidiário.
9. Listar documentos e revisão.
...
```json
{
  "skill": "ep-peca-saude-transferencia-protecao",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "minuta": "",
    "tipo_pedido": "saude | transferencia | protecao | apuracao | outro",
    "risco_concreto": "",
    "pedido_urgente": "",
    "providencias_operacionais": [],
    "documentos_citados": []
  },
  "premissas": [],
  "fontes_documentais": [],
...
A skill deve produzir pedido urgente de saúde, transferência ou proteção com padrão de criminalista sênior: objetivo, documentado, com cálculo quando aplicável, tese clara, enfrentamento de riscos e pedido operacional executável.
