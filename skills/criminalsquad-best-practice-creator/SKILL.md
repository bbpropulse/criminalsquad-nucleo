---
name: criminalsquad-best-practice-creator
description: >-
  Use para criar ou operar criminalsquad best practice creator: Guides creation and maintenance of
  best-practice files for the CriminalSquad best-practices library. Handles format validation,
  cross-references, versioning, and catalog consistency. Não use para mutação de produção,
  duplicação de capability ou dependência presumida sem validação.
metadata:
  description_pt-BR: >
    Guia a criação e manutenção de arquivos de best-practice na biblioteca de best-practices do CriminalSquad.
    Cuida de validação de formato, referências cruzadas, versionamento e consistência do catálogo.
  description_es: >
    Guía la creación y mantenimiento de archivos de best-practice en la biblioteca de best-practices de CriminalSquad.
  ...
  eval_case_ids: ["csq-v5-criminalsquad-best-practice-creator"]
  risk_level: "r2"
  delivery_type: "system-artifact"
  freshness_policy: "dependency-state-current"
  positive_triggers: ["criminalsquad-best-practice-creator", "criminalsquad best", "practice creator"]
...
Use this workflow when creating a new best-practice file for the `_criminalsquad/core/best-practices/` library.

## Pre-flight Checks

1. **Scan existing best-practice files**: Read `_criminalsquad/core/best-practices/_catalog.yaml`. Extract `id`, `name`, `whenToUse`, `file` from each entry.
...
Add a new entry to `_criminalsquad/core/best-practices/_catalog.yaml` with:
...
Save to `_criminalsquad/core/best-practices/{id}.md`.
...
Use this workflow when updating best-practice files in the `_criminalsquad/core/best-practices/` library.
...
2. Remove its entry from `_criminalsquad/core/best-practices/_catalog.yaml`
3. Scan ALL remaining best-practice files in `_criminalsquad/core/best-practices/*.md`
...
```
Read _criminalsquad/core/best-practices/_catalog.yaml
For each entry in catalog:
  1. Verify _criminalsquad/core/best-practices/{entry.file} exists
...
For each .md file in _criminalsquad/core/best-practices/ (excluding _catalog.yaml):
...
```
For each best-practice file A in _criminalsquad/core/best-practices/*.md:
  For each "NOT for: ... → See {id}" in A.whenToUse:
    1. Verify _criminalsquad/core/best-practices/{id}.md exists
