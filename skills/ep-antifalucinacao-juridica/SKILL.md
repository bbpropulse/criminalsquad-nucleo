---
name: ep-antifalucinacao-juridica
description: >-
  Use para bloquear fatos, datas, cálculos, leis, precedentes ou documentos inventados em análise ou
  peça de execução penal. Não use para conclusão definitiva sem autos suficientes, fonte atual ou
...
metadata:
  ...
  eval_case_ids: ["csq-v5-ep-antifalucinacao-juridica"]
...
Aplicar barreira final contra alucinação jurídica, documental e matemática, bloqueando entrega quando houver invenção ou baixa confiabilidade crítica.

# Quando usar

- Obrigatoriamente antes de entregar peça gerada por IA.
- Após redação de parecer ou minuta.
- Quando há jurisprudência, cálculo ou datas.
- Quando documentos estão incompletos.

# Entradas necessárias ou desejáveis

- Texto final ou análise.
- Fontes documentais.
- Cálculo.
...
1. Extrair fatos, datas, números, documentos, leis e precedentes citados.
2. Verificar se cada item tem fonte ou premissa.
3. Identificar citações sem documento.
4. Identificar cálculo sem fórmula/premissa.
5. Identificar jurisprudência sem fonte.
6. Identificar artigos ou súmulas possivelmente inexistentes/desatualizados.
7. Identificar mistura de processos ou clientes.
8. Classificar erro como bloqueante ou corrigível.
9. Gerar lista de correções obrigatórias.
10. Aprovar, aprovar com ressalvas ou bloquear.
...
```json
{
  "skill": "ep-antifalucinacao-juridica",
...
- Bloquear qualquer jurisprudência sem identificação verificável.
- Bloquear qualquer data decisiva sem fonte ou premissa.
- Bloquear conclusão de liberdade/benefício vencido sem cálculo auditável.
...
- Deve ser conservadora: na dúvida crítica, bloquear e pedir revisão humana.
...
A skill executa bloqueio de alucinações jurídicas. Seu papel é impedir protocolo de material fraco, perigoso ou contraditório. Deve apontar problemas com franqueza e classificar gravidade.
