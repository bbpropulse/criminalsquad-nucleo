---
name: ep-simulador-ministerio-publico
description: >-
  Use para simular objeções do Ministério Público contra análise, pedido ou recurso de execução
...
metadata:
  ...
  eval_case_ids: ["csq-v5-ep-simulador-ministerio-publico"]
...
Antecipar objeções acusatórias e institucionais para fortalecer peça ou estratégia defensiva.

# Quando usar

- Antes de protocolar pedido relevante.
- Antes de sustentação oral.
- Quando a tese é vulnerável.
- Quando há falta grave, laudo desfavorável, reincidência, gravidade do crime ou documento faltante.
...
1. Ler a tese como parte contrária.
2. Identificar ausência de requisito objetivo ou subjetivo.
3. Atacar fragilidades documentais.
4. Apontar riscos de inadequação da via.
5. Apontar precedente contrário ou interpretação restritiva a verificar.
6. Simular manifestação do MP em pontos centrais.
7. Sugerir resposta defensiva para cada objeção.
8. Classificar força da objeção.
9. Indicar ajustes na peça.
...
```json
{
  "skill": "ep-simulador-ministerio-publico",
...
- A simulação deve ser dura e útil, não caricatural.
...
A skill executa simulação de objeções do Ministério Público. Seu papel é impedir protocolo de material fraco, perigoso ou contraditório. Deve apontar problemas com franqueza e classificar gravidade.
