...
description: >-
  Use para preparar e executar, com checkpoint, image ai generator: Generates images via Openrouter
  API using AI image models. Supports two modes: test (cheap model for iteration) and production
  (high-quality model for final output). Handles prompt construction, API calls, base64 decoding,
  and file saving. Supports reference images (logos, mascots) for brand-consistent generation. Não
  use para envio, publicação, agendamento, assinatura ou alteração externa sem checkpoint explícito.
metadata:
  description_pt-BR: >
  ...
  eval_case_ids: ["csq-v5-image-ai-generator"]
  risk_level: "r4"
  delivery_type: "external-mutation"
  freshness_policy: "provider-state-current"
  positive_triggers: ["image-ai-generator", "image generator"]
