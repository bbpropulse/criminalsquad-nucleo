...
➡️ **Como abrir o arquivo para análise:** a skill `captura-midia-av` é a **etapa de captura** deste exame — registra hash/duração/codec, transcreve o áudio com timestamp e extrai os frames que você vai comparar quadro a quadro (`npx criminalsquad captura "<arquivo>" --sigiloso`; `--every SEG` para a varredura contínua que revela salto, repetição e corte; `--start/--end` para o trecho decisivo). Rode-a **sobre uma cópia**, jamais sobre o original.
...
- Cruzar com **frame estático** (`leitura-still-cftv-defesa`), **metadados** (`leitura-exif-metadados-imagem`), **comparação facial** (`leitura-comparacao-facial-reconhecimento-imagem`) e **laudo pericial de imagem** (`leitura-laudo-pericial-imagem`); a captura dos frames e da fala gravada vem de `captura-midia-av`.
