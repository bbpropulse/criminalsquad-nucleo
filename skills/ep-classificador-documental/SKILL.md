---
name: ep-classificador-documental
description: >-
  Use para classificar documentos de execução penal: sentença, guia, cálculo, atestado, PAD,
  decisões, laudos, relatórios e peças. Não use para conclusão de mérito sem validação, alteração do
...
metadata:
  ...
  eval_case_ids: ["csq-v5-ep-classificador-documental"]
...
Classificar documentos processuais e administrativos, atribuindo tipo, relevância, data, confiança e impacto jurídico.

# Quando usar

- Ao receber PDF integral ou documentos soltos.
- Quando o processo estiver desorganizado.
- Antes da extração de dados executórios.
- Quando o sistema precisar indexar documentos.

# Entradas necessárias ou desejáveis

- Arquivos, textos extraídos, imagens ou trechos documentais.
- Metadados de evento, página ou fonte, quando disponíveis.

# Protocolo obrigatório

1. Ler cada documento ou trecho.
2. Identificar tipo documental provável.
3. Extrair data, órgão emissor, assunto, evento e partes relevantes.
4. Classificar relevância jurídica para cálculo, benefício, falta, recurso, saúde ou direito preso.
5. Atribuir nível de confiança.
6. Marcar documentos duplicados ou incompletos.
7. Identificar documentos críticos ausentes.
8. Entregar índice documental estruturado.
...
```json
{
  "skill": "ep-classificador-documental",
...
- Não classificar documento ilegível com confiança alta.
- Não extrair conclusão jurídica definitiva apenas pela classificação.
...
- A classificação deve ser granular o suficiente para permitir citação posterior por documento.
...
A skill deve transformar documentos soltos em inventário jurídico. Classificar bem reduz alucinação nas etapas seguintes e evita que o cálculo use documento errado ou desatualizado.

## Perguntas críticas que a skill deve responder

- Que tipo de documento é este?
- Ele tem data e órgão emissor?
- Ele é decisão, certidão, cálculo, peça, laudo ou relatório?
- Ele altera liberdade, cálculo, regime, remição ou risco?
- É duplicado?
- Está legível?
- É documento atualizado ou antigo?
- Falta página?
- Há assinatura ou evento?
- Qual skill deve consumir este documento?
...
- prints
- textos extraídos
- metadados de upload/evento

## Protocolo ampliado de alta performance

1. Ler cada item
2. identificar tipo provável
3. extrair data
4. extrair origem
5. atribuir relevância
6. atribuir confiança
7. detectar duplicidade
8. detectar ausência de documento crítico
9. gerar índice

## Armadilhas e erros de alta gravidade

- cálculo antigo tratado como atual
- manifestação do MP confundida com decisão
- PAD incompleto
- laudo sem identificação
- documento ilegível com alta confiança
