---
name: ep-peca-mandado-seguranca-correcao
description: >-
  Use para redigir mandado de segurança ou correição parcial em execução penal quando cabíveis e
  mais adequados que HC/agravo. Não use para decisão final, assinatura, protocolo ou citação não
  verificada.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-peca-mandado-seguranca-correcao"]
...
Produzir minuta de mandado de segurança ou correição parcial para ato ilegal, abusivo, omissivo ou tumultuário em execução penal, quando a via for estrategicamente adequada.

# Quando usar

- Quando há ato ilegal sem ameaça direta de liberdade suficiente para HC.
- Quando há omissão, tumulto processual ou ato administrativo judicial atípico.
- Quando agravo não é suficiente ou não tem efeito útil.
- Quando o regimento local admite correição.

# Entradas necessárias ou desejáveis

- Ato coator ou omissão.
- Data da ciência/intimação.
- Prova pré-constituída.
- Regimento local, se for correição.
- Decisões e documentos relevantes.
- Objetivo da medida.

# Protocolo obrigatório

1. Verificar se HC ou agravo seriam vias mais adequadas.
2. Identificar direito líquido e certo ou tumulto processual.
3. Confirmar prova pré-constituída.
4. Verificar prazo e competência.
5. Definir autoridade coatora.
6. Redigir fatos essenciais.
7. Demonstrar ilegalidade/abuso/omissão.
8. Formular pedido liminar, se cabível.
9. Formular pedido final e operacional.
10. Listar riscos de inadequação da via.
...
```json
{
  "skill": "ep-peca-mandado-seguranca-correcao",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "minuta": "",
    "via": "mandado_de_seguranca | correicao_parcial",
    "ato_impugnado": "",
    "direito_ou_tumulto": "",
    "pedido_liminar": "",
    "pedidos": [],
...
A skill deve produzir mandado de segurança ou correição parcial com padrão de criminalista sênior: objetivo, documentado, com cálculo quando aplicável, tese clara, enfrentamento de riscos e pedido operacional executável.
...
- usar MS no lugar de recurso próprio sem justificar
- falta de prova pré-constituída
- não demonstrar omissão
- fato sem fonte
