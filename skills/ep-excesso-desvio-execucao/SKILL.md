---
name: ep-excesso-desvio-execucao
description: >-
  Use para identificar excesso ou desvio de execução: pena além do devido, regime mais gravoso,
  atraso em benefício, sanção ilegal ou alvará não cumprido. Não use para conclusão definitiva sem
...
metadata:
  ...
  category: incidentes, crise e risco
  version: "3.0.0"
  risk_level: "r3"
  input_types:
    - pad
  ...
  next_skills_sugeridas:
    ...
    - ep-peca-defesa-pad
    - ep-peca-agravo-execucao
  type: "prompt"
  schema_version: "5"
  quality_profile: "legal-analysis"
  contract_version: "5.0.0"
  quality_status: "contracted"
  eval_case_ids: ["csq-v5-ep-excesso-desvio-execucao"]
  delivery_type: "legal-analysis"
  freshness_policy: "official-current-source-required"
  positive_triggers: ["pad", "falta_grave", "regressao", "monitoracao", "exame_criminologico", "excesso", "crise"]
...
Detectar situações em que a execução ultrapassa o título judicial, a lei, o cálculo correto, o regime devido ou os limites constitucionais.

# Quando usar

- Quando há prisão além do término.
- Quando a pessoa cumpre regime mais gravoso.
- Quando benefício vencido não foi analisado.
- Quando decisão favorável não foi cumprida.
- Quando há sanção, isolamento ou transferência ilegal.
- Quando medida de segurança parece sem limite.

# Entradas necessárias ou desejáveis

- Cálculo e atestado de pena.
- Decisões de regime e benefícios.
- Eventos do SEEU.
- Informações de unidade e vaga.
- Decisão de sanção, isolamento ou transferência.
- Laudos e documentos, se for medida de segurança ou saúde.

# Protocolo obrigatório

1. Identificar o título executivo e seus limites.
2. Comparar pena, regime e condições legais com a realidade cumprida.
3. Identificar excesso quantitativo, qualitativo ou procedimental.
4. Mensurar tempo de excesso ou gravidade da violação.
5. Identificar decisão omitida, descumprida ou inexistente.
6. Verificar urgência e dano atual.
7. Indicar instrumento adequado: petição, HC, agravo, MS, correição ou providência administrativa.
8. Formular pedido operacional de correção.
9. Listar documentos indispensáveis.
10. Entregar parecer de excesso/desvio.
...
```json
{
  "skill": "ep-excesso-desvio-execucao",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "excesso_desvio": {
      "tipo": "quantitativo | qualitativo | procedimental | estrutural",
      "fato_gerador": "",
      "prova": [],
      "impacto": "",
...
- A saída deve transformar violação em providência concreta e mensurável.
...
# Doutrina operacional da categoria: incidentes, crise e risco

As skills desta categoria lidam com perda de liberdade, regressão, sanção, falta grave, laudo negativo, tornozeleira e excesso. Devem operar com urgência, contraditório, proporcionalidade e prova.

## Regras específicas da categoria

- Separar providência cautelar de decisão definitiva.
- Verificar contraditório e defesa técnica.
- Verificar individualização da conduta.
- Exigir cópia integral do PAD, relatório de monitoração ou laudo quando relevante.
- Apontar efeito jurídico exato do incidente: data-base, remição, regime, benefício, subjetivo ou alvará.
- Nunca aceitar automatismo punitivo sem base legal e decisão fundamentada.


# Núcleo técnico avançado desta skill

A skill deve detectar execução fora do título ou mais gravosa que o permitido, tanto em tempo quanto em qualidade.

## Perguntas críticas que a skill deve responder

- A pena cumprida supera o devido?
- O regime é mais gravoso?
- Há decisão favorável não cumprida?
- Há atraso injustificado em benefício?
- Há sanção sem decisão?
- Há isolamento irregular?
- Há medida de segurança sem limite?
- Há restrição de visita/saúde/trabalho ilegal?
- Qual é a providência imediata?
- Cabe HC?
...
1. Classificar excesso quantitativo/qualitativo
2. identificar título executivo
3. comparar execução real
4. comprovar desvio
5. medir urgência
6. definir pedido e instrumento
7. preparar quadro de excesso

## Armadilhas e erros de alta gravidade

- prisão além do término
- regime fechado após progressão
- alvará parado
- sanção coletiva
- demora estrutural sem providência
...
Quando houver incidente com risco de regressão, perda de remição, isolamento, PAD, laudo negativo ou descumprimento de monitoramento, a skill deve gerar:

```json
{
  "fato_critico": "",
  "risco_imediato": "",
  "prova_necessaria_em_24h": [],
  "tese_de_contenção": "",
  "providencia_urgente": "",
  "instrumento_possivel": "peticao | agravo | habeas_corpus | pedido_administrativo | outro"
}
```

## Padrão defensivo em crise

1. Controlar narrativa fática.
2. Exigir documento integral.
3. Separar fato de consequência.
4. Impugnar automatismo.
5. Propor medida menos gravosa.
6. Preservar cálculo e remição quando juridicamente possível.
