---
name: ep-cenarios-calculo-comparativo
description: >-
  Use para gerar cenários alternativos de cálculo quando houver controvérsia sobre fração,
  data-base, remição, falta grave ou lei aplicável. Não use para escolha automática de lei, fração,
...
metadata:
  ...
  eval_case_ids: ["csq-v5-ep-cenarios-calculo-comparativo"]
...
Criar cenários comparativos de cálculo para permitir decisão estratégica diante de incerteza jurídica ou documental.

# Quando usar

- Quando houver mais de uma interpretação plausível.
- Quando a jurisprudência estiver dividida ou pendente.
- Quando faltarem documentos para conclusão definitiva.
- Quando o advogado quiser comparar risco e benefício.
...
- Premissas possíveis.
- Cálculo oficial.
- Eventos controvertidos.
- Teses jurídicas possíveis.

# Protocolo obrigatório

1. Identificar a variável controvertida.
2. Criar cenário conservador com premissas restritivas.
3. Criar cenário defensivo com premissas favoráveis.
4. Criar cenário intermediário, se útil.
5. Calcular progressão, livramento e término em cada cenário.
6. Apontar impacto em dias.
7. Classificar risco jurídico e probatório de cada cenário.
8. Indicar qual cenário usar em peça principal e qual usar como subsidiário.
9. Entregar tabela comparativa.
...
```json
{
  "skill": "ep-cenarios-calculo-comparativo",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "cenarios": [
      {
        "nome": "conservador | defensivo | intermediario",
        "premissas": [],
        "data_progressao": "",
        "data_livramento": "",
        "termino": "",
...
- A saída deve deixar claro que cenário não é fato, mas sim cálculo condicionado a premissas.
...
Quando a tese é controvertida, a skill deve produzir cenários. Isso permite estratégia: pedido principal, subsidiário e estimativa de risco.

## Perguntas críticas que a skill deve responder

- Qual premissa está em disputa?
- Há dúvida de fração?
- Há dúvida de data-base?
- Há remição pendente?
- Há falta grave discutível?
- Há lei nova?
- Qual cenário é oficial?
- Qual cenário é defensivo?
- Qual cenário é conservador?
- Qual impacto em dias?

## Documentos e dados específicos a procurar

- cálculo oficial
- documentos que sustentam premissas
- jurisprudência atualizada
- decisões incidentais

## Protocolo ampliado de alta performance

1. Identificar variável controversa
2. definir cenário oficial
3. definir cenário defensivo
4. definir cenário conservador
5. calcular datas em cada cenário
6. comparar impacto
7. classificar risco
8. sugerir tese/pedido

## Armadilhas e erros de alta gravidade

- misturar premissas de cenários diferentes
- não explicar diferença
- escolher cenário favorável sem risco
- usar cenário controvertido como certeza
