---
name: ep-dashboard-carteira
description: >-
  Use para transformar múltiplos casos de execução penal em painel de carteira com prioridades,
  benefícios, riscos e próximas ações. Não use para compartilhamento de dados, promessa de resultado
  ou decisão jurídica sem autorização.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-dashboard-carteira"]
...
Gerar estrutura de acompanhamento de carteira para execução penal, com prioridades, prazos internos, benefícios, incidentes e status de cada caso.

# Quando usar

- Ao revisar carteira do escritório.
- Quando há vários clientes presos.
- Antes de mutirão de benefícios, remição ou indulto.
- Para alimentar CRM, planilha ou sistema interno.

# Entradas necessárias ou desejáveis

- Lista de casos.
- Dados executórios resumidos.
- Cálculos e próximos benefícios.
- PADs, remições e urgências.
- Status de peças e protocolos.

# Protocolo obrigatório

1. Padronizar dados de cada cliente.
2. Identificar regime, pena, data-base e próximos benefícios.
3. Classificar urgência e prioridade.
4. Mapear remições e PADs pendentes.
5. Mapear saúde, transferência ou proteção.
6. Mapear prazos recursais e prazos internos.
7. Definir próxima ação por caso.
8. Gerar tabela estruturada.
9. Sinalizar dados faltantes.
...
- O dashboard deve ser acionável, não apenas descritivo.
...
A skill deve transformar dashboard de carteira em rotina de alta performance. O foco é reduzir esquecimento, padronizar qualidade, criar ação e preservar comunicação clara.
