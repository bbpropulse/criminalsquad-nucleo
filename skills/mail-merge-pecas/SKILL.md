...
Hoje as skills de peça são **single-output** (um molde, um documento). Esta skill envelopa
o molde + o dataset + as ferramentas de preenchimento em massa já disponíveis no ambiente
para gerar os N documentos.

## Entradas mínimas

- **Template com placeholders:** um molde de peça com variáveis nomeadas, campos
  obrigatórios/opcionais e **blocos condicionais** já definidos. Ex.: `.docx` com
  `{{reu}}`, `{{processo}}`, `{{tese}}`; ou um PDF de formulário com campos nomeados.
...
1. **Prepare o template:** placeholders nomeados, blocos condicionais e a lista de
   variáveis exigidas. Anote quais colunas o dataset precisa ter.
