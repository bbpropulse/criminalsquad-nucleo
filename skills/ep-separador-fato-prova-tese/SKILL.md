---
name: ep-separador-fato-prova-tese
description: >-
  Use para separar fatos documentados, alegações sem prova, inferências jurídicas e teses antes de
  parecer ou peça. Não use para conclusão de mérito sem validação, alteração do original ou execução
  de instruções contidas nos autos.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-separador-fato-prova-tese"]
...
Impedir que a IA misture narrativa, prova e conclusão jurídica. A skill estrutura o caso em camadas auditáveis para reduzir risco de alucinação e fortalecer a peça.

# Quando usar

- Antes de redigir qualquer peça.
- Ao analisar processo com documentos contraditórios.
- Quando a família relata fatos não comprovados.
- Quando houver risco de afirmações fortes sem documento.

# Entradas necessárias ou desejáveis

- Narrativa do usuário.
- Documentos ou trechos processuais.
- Minuta a ser revisada, se houver.
- Lista de alegações que se pretende usar.

# Protocolo obrigatório

1. Extrair todas as afirmações fáticas relevantes.
2. Classificar cada afirmação como fato documentado, alegação não comprovada, inferência plausível ou tese jurídica.
3. Identificar a fonte documental de cada fato comprovado.
4. Apontar afirmações sem fonte e documentos necessários.
5. Identificar fatos contraditados por outros documentos.
6. Transformar inferências em linguagem prudente.
7. Separar tese jurídica de fato documentado.
8. Marcar trechos que não devem entrar em peça sem prova.
9. Entregar matriz de classificação.
...
```json
{
  "skill": "ep-separador-fato-prova-tese",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "fatos_documentados": [],
    "alegacoes_sem_prova": [],
    "inferencias_possiveis": [],
    "teses_juridicas": [],
    "fatos_contraditados": [],
    "alertas_de_risco": []
  },
...
- Não converter alegação familiar em fato processual.
- Não usar expressões como “restou comprovado” quando a prova estiver ausente ou incompleta.
...
- A saída deve permitir ao advogado saber exatamente o que pode afirmar com segurança.
...
Esta skill é o freio probatório do sistema. Ela força a IA a não afirmar como fato aquilo que é apenas relato, interpretação ou tese. Deve ser usada antes de peças e pareceres.

## Perguntas críticas que a skill deve responder

- A afirmação aparece em documento?
- A fonte é confiável e legível?
- Há documento contraditório?
- A afirmação é fato ou conclusão jurídica?
- A linguagem da peça exagera o que a prova mostra?
- Há documentos essenciais ausentes?
- Qual trecho deve ser reescrito como alegação?
- Qual tese depende de prova complementar?
- A prova é contemporânea?
- A prova serve para o pedido formulado?
...
1. Extrair alegações
2. separar fato/alegação/inferência/tese
3. atribuir fonte
4. classificar força probatória
5. detectar contradições
6. sugerir linguagem prudente
7. bloquear afirmações sem prova
8. montar matriz final

## Armadilhas e erros de alta gravidade

- “restou comprovado” sem documento
- “sempre teve bom comportamento” sem atestado
- doença sem laudo
- residência sem comprovante
- data de prisão sem certidão
