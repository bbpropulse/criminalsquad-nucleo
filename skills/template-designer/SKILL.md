...
description: >-
  Use para criar ou operar template designer: Visual template selection for image design agents.
  Generates template variations, renders them as images for user review, and saves the approved
  visual identity. Não use para mutação de produção, duplicação de capability ou dependência
...
metadata:
  ...
  eval_case_ids: ["csq-v5-template-designer"]
  risk_level: "r2"
  delivery_type: "system-artifact"
  freshness_policy: "dependency-state-current"
  positive_triggers: ["template-designer", "template designer"]
...
- `_criminalsquad/core/best-practices/image-design.md` — **REQUIRED reading**. Contains platform-specific minimum font sizes, typography rules, spacing guidelines, color palette constraints, contrast requirements, and layout methodology. Every template you generate MUST comply with these rules.
...
- `_criminalsquad/_memory/company.md` — company name, brand, industry, target audience
- `_criminalsquad/_memory/preferences.md` — user preferences (language, style, tone)
...
**Important:** Always use the full absolute path (e.g., `d:\Coding Projects\criminalsquad\squads\my-squad\_build\template-a.png`) inside the markdown link — relative paths are not clickable in the IDE.
