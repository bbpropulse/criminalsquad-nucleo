---
name: ep-indulto-comutacao
description: >-
  Use para analisar indulto ou comutação com base no decreto aplicável, requisitos, impedimentos e
  cálculo pós-comutação. Não use para conclusão definitiva sem autos suficientes, fonte atual ou
...
metadata:
  ...
  eval_case_ids: ["csq-v5-ep-indulto-comutacao"]
...
Avaliar clemência penal de forma técnica, lendo o decreto aplicável, testando requisitos, exclusões, datas de referência, faltas e impacto no cálculo.

# Quando usar

- Após publicação de decreto de indulto/comutação.
- Na revisão anual da carteira.
- Quando o cliente pode se enquadrar por tempo, idade, saúde, deficiência, maternidade ou pena remanescente.
- Quando houve indeferimento de indulto/comutação.

# Entradas necessárias ou desejáveis

- Decreto aplicável integral e atualizado.
- Cálculo de pena.
- Condenações e natureza dos crimes.
- Certidão disciplinar.
- Faltas graves e datas.
- Laudos de saúde ou documentos de vulnerabilidade, se houver.
- Decisões anteriores.

# Protocolo obrigatório

1. Ler integralmente o decreto informado.
2. Extrair requisitos e impedimentos.
3. Identificar data de referência.
4. Confrontar cada condenação com exclusões do decreto.
5. Verificar tempo cumprido, pena remanescente e fração/percentual de comutação.
6. Verificar faltas graves no período relevante conforme decreto.
7. Calcular efeito da comutação, se cabível.
8. Indicar tese contra impedimento genérico, se houver base.
9. Listar documentos necessários.
10. Entregar parecer de cabimento e providência recomendada.
...
```json
{
  "skill": "ep-indulto-comutacao",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "indulto_comutacao": {
      "decreto": "",
      "cabimento": "indulto | comutacao | nao_cabe | depende",
...
- Não analisar indulto/comutação sem o texto do decreto aplicável ou fonte oficial atualizada.
- Não presumir que decreto de ano anterior se aplica ao caso atual.
...
- A análise deve ser literal em relação ao decreto, mas estratégica nas teses de interpretação.
...
Indulto e comutação dependem do decreto aplicável. A skill deve ler o decreto vigente, extrair requisitos e aplicar ao caso com cálculo antes/depois.

## Perguntas críticas que a skill deve responder

- Qual decreto aplicável?
- Qual data de referência?
- O crime é incluído ou excluído?
- Qual requisito temporal?
- Há falta grave no período relevante?
- Há condição humanitária?
- Cabe indulto total?
- Cabe comutação?
- Qual pena remanescente?
- Qual cálculo pós-comutação?
...
- sentença/guia
- certidão disciplinar
- documentos de saúde/idade/deficiência se relevantes
- decisões anteriores
...
3. extrair impedimentos
4. aplicar ao caso
5. validar cálculo
6. calcular comutação
7. listar documentos
8. gerar tese
9. indicar peça
...
- usar decreto de ano errado
- ignorar data de referência
- ignorar crime excluído
- usar falta grave fora do período
- não recalcular pena após comutação
