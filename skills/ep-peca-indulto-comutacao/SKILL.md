---
name: ep-peca-indulto-comutacao
description: >-
  Use para redigir pedido de indulto ou comutação com base no decreto aplicável e cálculo de
  impacto. Não use para decisão final, assinatura, protocolo ou citação não verificada.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-peca-indulto-comutacao"]
...
Produzir minuta de pedido de indulto/comutação demonstrando enquadramento no decreto, ausência de impedimentos e consequência executória.

# Quando usar

- Quando análise de decreto indicou cabimento.
- Quando há indeferimento a impugnar.
- Quando comutação altera cálculo e benefícios.
- Quando indulto pode extinguir pena.

# Entradas necessárias ou desejáveis

- Texto do decreto aplicável.
- Parecer de indulto/comutação.
- Cálculo de pena.
- Condenações e crimes.
- Certidão disciplinar.
- Documentos de saúde/vulnerabilidade se o pedido depender deles.

# Protocolo obrigatório

1. Identificar decreto e dispositivo aplicável.
2. Apresentar dados executórios essenciais.
3. Demonstrar requisitos preenchidos.
4. Enfrentar impedimentos aparentes.
5. Demonstrar falta grave ou ausência dela conforme decreto.
6. Calcular comutação, se aplicável.
7. Pedir indulto/extinção ou comutação/recálculo.
8. Pedir atualização do atestado e análise de benefícios derivados.
9. Listar documentos citados.
10. Marcar pontos de revisão humana.
...
```json
{
  "skill": "ep-peca-indulto-comutacao",
  "status": "concluido | concluido_com_ressalvas | bloqueado",
  "resultado": {
    "minuta": "",
    "decreto": "",
    "enquadramento": "",
    "impedimentos_enfrentados": [],
    "calculo_pos_comutacao": "",
    "pedidos": [],
    "documentos_citados": []
  },
  "premissas": [],
...
A skill deve produzir pedido de indulto ou comutação com padrão de criminalista sênior: objetivo, documentado, com cálculo quando aplicável, tese clara, enfrentamento de riscos e pedido operacional executável.
...
- usar decreto errado
- não verificar crime excluído
- não calcular consequência
- fato sem fonte
