---
name: ep-alertas-beneficios-prazos
description: >-
  Use para gerar alertas preventivos de benefícios, prazos recursais, remição, PAD, exames, saúde e
  providências. Não use para compartilhamento de dados, promessa de resultado ou decisão jurídica
  sem autorização.
metadata:
  lifecycle: "preview"
  ...
  eval_case_ids: ["csq-v5-ep-alertas-beneficios-prazos"]
...
Criar alertas preventivos para evitar perda de prazos, atraso em benefícios e esquecimento de providências executórias.

# Quando usar

- Após auditoria de cálculo.
- Ao alimentar dashboard.
- Quando há benefícios próximos.
- Quando há prazos recursais ou providências pendentes.

# Entradas necessárias ou desejáveis

- Dados do caso ou carteira.
- Cálculo com datas futuras.
- Prazos processuais.
- Eventos de PAD, remição, exame, saúde ou alvará.
- Política interna de antecedência.

# Protocolo obrigatório

1. Identificar datas críticas.
2. Criar alertas antes do vencimento conforme tipo de evento.
3. Gerar alertas para coleta de documentos antes do benefício.
4. Gerar alertas de prazo recursal.
5. Gerar alertas de reavaliação de saúde, monitoração ou exame.
6. Gerar alertas de indulto/comutação anual.
7. Classificar alerta por prioridade.
8. Indicar responsável e próxima ação, se fornecido.
9. Entregar lista estruturada.
...
```json
{
  "skill": "ep-alertas-beneficios-prazos",
...
- Cada alerta deve ter uma ação concreta, não apenas uma data.
...
A skill deve transformar alertas de benefícios e prazos em rotina de alta performance. O foco é reduzir esquecimento, padronizar qualidade, criar ação e preservar comunicação clara.
