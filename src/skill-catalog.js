...
import { listPacks, packContentDir, packLabel } from './packs-overlay.js';
...
/**
 * Skills que chegaram pelo sync: _criminalsquad/_packs/<pack>/skills/<id>/SKILL.md.
 *
 * Elas ENTRAM no índice (senão o download fica invisível para a busca), mas não
 * passam pelo gate estrutural de `check-skills`: o pacote é assinado pela
 * curadoria, que roda o gate antes de publicar, e um defeito lá não pode
 * quebrar o catálogo do usuário. Uma skill sincronizada nunca sombreia uma
 * embarcada de mesmo id — o que veio no pacote npm continua sendo a canônica.
 */
export function discoverPackSkills(projectDir, { excludeIds = new Set() } = {}) {
  const entries = [];
  const descartadas = [];
  const seen = new Set();
  for (const pack of listPacks(projectDir)) {
    const packSkillsDir = packContentDir(pack, 'skills');
    if (!packSkillsDir) continue;
    const ids = readdirSync(packSkillsDir, { withFileTypes: true })
      .filter((entry) => entry.isDirectory() && !entry.name.startsWith('_'))
      .map((entry) => entry.name)
      .sort((a, b) => a.localeCompare(b));
    for (const id of ids) {
      if (excludeIds.has(id) || seen.has(id)) continue;
      const skillPath = join(packSkillsDir, id, 'SKILL.md');
      if (!existsSync(skillPath)) continue;
      const raw = readFileSync(skillPath, 'utf8');
      const frontmatter = extractFrontMatter(raw);
      const metadata = parseSkillMetadata(raw, { fallbackName: id });
      // Defeito no pacote NÃO pode derrubar o catálogo do usuário: sem
      // frontmatter, a auditoria estouraria e levaria junto indexar/buscar/
      // resolver. A skill é descartada com aviso — o resto continua de pé.
      if (!frontmatter || !metadata) {
        descartadas.push({ id, skillPath, motivo: 'frontmatter ausente ou inválido' });
        continue;
      }
      seen.add(id);
      entries.push({
        ...
        frontmatter,
        metadata,
        policy: getSkillLifecyclePolicy(metadata.lifecycle),
        group: classifySkillGroup(metadata.categories || [], metadata.type || 'prompt', id),
        fontePack: packLabel(pack),
        verificadoEm: pack.verificadoEm,
      });
    }
  }
  return { entries, descartadas };
}

/** Bibliotecas de skills sincronizadas — onde moram os evals e a evidência do pacote. */
export function packSkillsDirs(projectDir) {
  return listPacks(projectDir)
    .map((pack) => packContentDir(pack, 'skills'))
    .filter(Boolean);
}

/**
 * Catálogo com as skills sincronizadas mescladas. Não muta o original: devolve
 * um novo objeto (ou o mesmo, quando não há nada sincronizado).
 */
export function withPackSkills(catalog, projectDir) {
  // Calculado sempre: um pacote pode trazer só evals/evidência (nenhuma skill
  // nova) e a auditoria precisa enxergá-los mesmo assim.
  const dirsDosPacotes = packSkillsDirs(projectDir);
  const { entries: packEntries, descartadas } = discoverPackSkills(projectDir, {
    // missingSkillFiles entra no exclude: uma pasta local quebrada (skills/<id>/
    // sem SKILL.md) é um erro a consertar, não uma vaga a ser ocupada pelo
    // pacote — senão o defeito some da vista atrás do conteúdo sincronizado.
    excludeIds: new Set([
      ...catalog.entries.map((entry) => entry.id),
      ...(catalog.missingSkillFiles || []),
    ]),
  });
  if (!packEntries.length && !dirsDosPacotes.length) return catalog;
  const entries = packEntries.length
    ? [...catalog.entries, ...packEntries].sort((a, b) => {
      const group = GROUP_ORDER.indexOf(a.group) - GROUP_ORDER.indexOf(b.group);
      return group || a.id.localeCompare(b.id);
    })
    : catalog.entries;
  return {
    ...
    entries,
    packEntries,
    packSkillsDirs: dirsDosPacotes,
    packSkillsDescartadas: descartadas,
  };
}
...
export function renderSkillIndex(catalog) {
  ...
  yaml += '# Entrada com `fonte_pack` veio do sync (_criminalsquad/_packs/), não do pacote npm.\n';
  ...
  for (const [group, entries] of groups) {
    ...
    for (const entry of entries) {
      ...
      // Procedência do sync: presente só em skill que veio de pacote assinado.
      // `verificado_em` NÃO entra: é a hora de parede da instalação, então dois
      // computadores com o mesmo pacote gerariam índices diferentes e o índice
      // deixaria de ser determinístico. Quem quer a data usa `status`.
      if (entry.fontePack) yaml += `    fonte_pack: ${yamlString(entry.fontePack)}\n`;
    }
  }
  return yaml;
}
...
export function validateSkillCatalog({
  ...
  // O índice esperado inclui o que veio do sync; as validações estruturais
  // abaixo, não — o pacote assinado responde pelo próprio gate.
  const expectedIndex = renderSkillIndex(withPackSkills(catalog, dirname(skillsDir)));
  const errors = [];
  const warnings = [];
