...
import { discoverSkillCatalog, withPackSkills } from './skill-catalog.js';
...
export function searchSkillCatalog(query, rootDir, options = {}) {
  ...
  const catalog = withPackSkills(discoverSkillCatalog(skillsDir), rootDir);
  ...
  const ranked = catalog.entries
    ...
    .map(({ entry, quality, match, rank }) => ({
      ...
      // Procedência: preenchido só quando a skill veio do sync da curadoria.
      // `skill_path` é obrigatório para ela: a sincronizada NÃO mora em
      // skills/<id>/SKILL.md, e sem o caminho quem consome a shortlist tentaria
      // abrir um arquivo que não existe.
      fonte_pack: entry.fontePack || null,
      skill_path: relative(rootDir, entry.skillPath).replaceAll('\\', '/'),
    }));
...
export function skillSearchCli(query, targetDir, values = {}) {
  ...
  for (const item of result.results) {
    ...
    console.log(`  - ${item.id} — ${gate}${pilot}${sync} — ${item.description}`);
  }
  return result;
}
