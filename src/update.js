...
const PROTECTED_PATHS = [
  ...
  '_criminalsquad/_packs', // conteúdo sincronizado (curadoria); o update do npm não pode apagá-lo
...
// `acervo` is protected wholesale (see PROTECTED_PATHS) so the update never touches a
// mentee's own materials or casos sigilosos. Jurisprudência e legislação são diferentes:
// são texto oficial que ninguém edita à mão, então conteúdo novo adicionado depois deve
// alcançar instalações que já existiam — mas SÓ como adição. Um arquivo que o(a)
// mentee já tem (vindo do init ou adicionado depois) nunca é tocado, sobrescrito ou removido.
const ADDITIVE_ONLY_DIRS = ['acervo/jurisprudencia', 'acervo/legislacao'];

async function syncAdditiveDirs(targetDir) {
  let count = 0;
  for (const relDir of ADDITIVE_ONLY_DIRS) {
    const srcDir = join(TEMPLATES_DIR, relDir);
    let entries;
    try {
      entries = await getTemplateEntries(srcDir);
    } catch {
      continue; // this template version doesn't ship the dir
    }
    for (const entry of entries) {
      const relPath = relative(srcDir, entry).replaceAll('\\', '/');
      const displayPath = `${relDir}/${relPath}`;
      const destPath = join(targetDir, relDir, relPath);
      try {
        await stat(destPath);
        continue; // já existe — nunca sobrescrever nem tocar
      } catch {
        // ausente — entregar
      }
      await mkdir(dirname(destPath), { recursive: true });
      await cp(entry, destPath);
      console.log(`  ${t('createdFile', { path: displayPath })}`);
      count++;
    }
  }
  return count;
...
export async function update(targetDir) {
  ...
  // 6c. Deliver new jurisprudência files the mentee doesn't have yet (additive-only —
  // see syncAdditiveDirs for why this is safe despite `acervo` being protected above).
  count += await syncAdditiveDirs(targetDir);
