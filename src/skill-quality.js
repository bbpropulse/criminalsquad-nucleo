...
const PROFILE_PATTERNS = [
  ...
  {
    // Wrappers não-jurídicos (site/slides/design) que delegam pra skills gerais
    // do Claude Code. Sem este override, "imagem"/"pdf"/"dashboard" nas
    // categories cairiam em evidence-forensics/client-operations por keyword
    // antes de chegar na regra genérica de system-orchestration (linha ~90).
    profile: 'system-orchestration',
    pattern: /^(?:site-creator|slides-creator|design-creator)(?:\s|$)/i,
  },
  {
...
// `extraSkillsDirs` são as bibliotecas sincronizadas (_packs/<pack>/skills). Sem
// elas, uma skill que chega pelo sync teria `eval_case_ids` apontando para casos
// que não existem localmente e ficaria bloqueada para sempre pelo gate.
//
// Precedência (a MESMA de discoverPackSkills, para o sistema não ter duas
// regras): o embarcado vence sobre qualquer pacote; entre pacotes, vence o
// primeiro em ordem de pack_id. Como `Map.set` faz o último sobrescrever, a
// iteração é: pacotes ao contrário, e o local por último.
export function loadSkillEvalCases(skillsDir, extraSkillsDirs = []) {
  const cases = new Map();
  for (const dir of [...extraSkillsDirs].reverse().concat(skillsDir)) {
    const evalsDir = join(dir, '_evals');
...
/**
 * Vínculo criptográfico entre a evidência e os bytes da SKILL — canônico quanto
 * ao próprio rótulo de maturidade.
 *
 * Medido: hashear o arquivo cru tornava a promoção IMPOSSÍVEL. A evidência
 * amarra o SHA-256 do SKILL.md, mas promover é editar `quality_status` dentro
 * desse mesmo arquivo — o hash mudava, a evidência recém-produzida virava
 * `skill_binding.skill_sha256 divergente` e a skill caía em hard fail. A única
 * saída era religar a evidência aos bytes já promovidos, passo indistinguível de
 * fraude (permitiria reescrever a skill inteira e religar). Nenhuma das 520
 * skills jamais saiu de `contracted`; este era o motivo mecânico.
 *
 * Removendo APENAS a linha `quality_status` do frontmatter, virar o rótulo não
 * mexe no vínculo, e qualquer outra alteração — corpo, versão, risco, descrição
 * — continua invalidando a evidência.
 */
export function computeSkillBindingSha256(raw) {
  const texto = String(raw ?? '');
  const match = texto.match(/^---\n([\s\S]*?)\n---/);
  const canonico = match
    ? texto.replace(match[0], `---\n${match[1].split('\n').filter((linha) => !/^\s*quality_status\s*:/.test(linha)).join('\n')}\n---`)
    : texto;
  return createHash('sha256').update(canonico, 'utf8').digest('hex');
}

export function readSkillEvidenceBinding(skillsDir, skill, extraSkillsDirs = []) {
  if (!/^[a-z0-9][a-z0-9-]*$/.test(String(skill || ''))) return null;
  // O embarcado é procurado PRIMEIRO: a evidência tem de casar com o SKILL.md
  // que o usuário realmente vai executar, e o embarcado sempre vence.
  const skillPath = [skillsDir, ...extraSkillsDirs]
  ...
  return {
    algorithm: 'sha256',
    skill_sha256: computeSkillBindingSha256(raw),
...
export function loadSkillEvaluationEvidence(skillsDir, extraSkillsDirs = []) {
  const evidence = new Map();
  const files = [...extraSkillsDirs, skillsDir]
    .flatMap((dir) => evaluationResultFiles(join(dir, '_evals', 'results')))
    .sort();
  for (const path of files) {
    ...
    for (const result of suite.results || []) {
      ...
      const binding = readSkillEvidenceBinding(skillsDir, skill, extraSkillsDirs);
...
export function evaluateSkillQuality(entry, {
  ...
  // Frontmatter ilegível (BOM, cerca ausente, YAML quebrado) chega aqui como
  // null. Sem esta normalização, o parse estourava TypeError e derrubava a
  // auditoria INTEIRA por causa de um arquivo — sem sequer dizer qual. Agora a
  // skill defeituosa apenas colhe hard fail estrutural, que é o veredito certo.
  const frontmatter = entry.frontmatter || '';
  const profileId = parseScalar(frontmatter, 'quality_profile')
    || classifySkillQualityProfile(entry);
  const profile = profiles.profiles[profileId];
  const body = bodyFromRaw(entry.raw);
  const description = entry.metadata?.description || '';
  const topLevelKeys = frontmatterTopLevelKeys(frontmatter);
  ...
  const evalIds = parseList(frontmatter, 'eval_case_ids');
  ...
  const qualityStatus = (parseScalar(frontmatter, 'quality_status') || 'legacy').toLowerCase();
...
export function auditSkillCatalogQuality(catalog, options = {}) {
  ...
  // Bibliotecas sincronizadas trazem os próprios evals e a própria evidência de
  // forward-run — o pacote é assinado pela mesma curadoria que publica o npm.
  const packDirs = catalog.packSkillsDirs || [];
  const evalCases = options.evalCases || loadSkillEvalCases(catalog.skillsDir, packDirs);
  const evidence = options.evidence || loadSkillEvaluationEvidence(catalog.skillsDir, packDirs);
