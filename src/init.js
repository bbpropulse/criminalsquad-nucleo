...
import { discoverSkillCatalog, renderSkillIndex, withPackSkills } from './skill-catalog.js';
...
// O critério para uma IDE entrar aqui é sempre o mesmo, e é técnico: a orquestração
// depende de TRÊS recursos — subagente em contexto isolado (o revisor que não é o
// redator, o juiz do smoke-run), hook determinístico que bloqueia ANTES da escrita
// (o Citation Gate) e skills. Sem subagente não há orquestração: entrega-se a casca
// e chama-se de suporte.
//
// O corte original deixou só o Claude Code, porque a contagem da época dava
// claude-code 34 agentes; codex e antigravity, 2; cursor, trae, gemini-cli,
// opencode, qwen-code e vscode-copilot, ZERO.
//
// O Cursor voltou em 28/07/2026 porque a versão 2.4 passou a ter os três:
// `.cursor/agents/*.md` com janela de contexto própria, `.cursor/hooks.json` com
// `preToolUse` que nega por `permission: "deny"` ou exit 2 (o MESMO mecanismo do
// Claude Code — o hook é um só, com o nome do evento normalizado) e `SKILL.md`.
// As demais seguem fora pelo mesmo critério, não por preferência.
const IDES = [
...
export async function init(targetDir, options = {}) {
  ...
  await copyJurisprudenciaAcervo(targetDir);
...
`;
  ...
  // A curadoria (o conteúdo publicado DEPOIS desta instalação) chega só por
  // `ativar` + `sync`, e nada no resto do produto anuncia que ela existe. Sem
  // estas duas linhas o aluno termina o init sem nunca descobrir que há material
  // novo para baixar — e a primeira delas evita o mal-entendido oposto, o de
  // achar que sem licença ficou com o sistema pela metade.
  console.log(`  ${t('syncCuradoria')}`);
  console.log(`  ${t('syncCuradoriaAtivar')}\n`);
}
...
export async function syncSkillCatalogArtifacts(targetDir, { overwriteManifest = false, backupFn = null } = {}) {
  ...
  const rendered = renderSkillIndex(
    withPackSkills(discoverSkillCatalog(targetSkills), targetDir),
  );
  let previous = null;
  try {
...
// Jurisprudência (official case-law text, thousands of small files) is excluded from
// the per-file walk below and delivered instead by copyJurisprudenciaAcervo() with a
// single recursive copy — see that function for why.
const JURISPRUDENCIA_REL = join('acervo', 'jurisprudencia');
...
async function copyCommonTemplates(targetDir) {
  ...
  for (const entry of entries) {
    ...
    if (normalized.includes('/acervo/jurisprudencia/')) continue;
...
// Thousands of small case-law files make the per-file walk above (mkdir+stat+cp+log
// EACH) take several real seconds and print a line per file. A single recursive copy
// does the same end result in one syscall-level operation instead of ~7k of them.
async function copyJurisprudenciaAcervo(targetDir) {
  const srcDir = join(TEMPLATES_DIR, JURISPRUDENCIA_REL);
  const destDir = join(targetDir, JURISPRUDENCIA_REL);
  try {
    await stat(destDir);
    return; // already present — never overwrite (mirrors the skip-if-exists rule above)
  } catch {
    // does not exist — copy it
  }
  let entries;
  try {
    entries = await getTemplateEntries(srcDir);
  } catch {
    return; // this template version doesn't ship the dir
  }
  if (entries.length === 0) return;
  await mkdir(dirname(destDir), { recursive: true });
  await cp(srcDir, destDir, { recursive: true });
  console.log(`  ${t('createdBulk', { path: JURISPRUDENCIA_REL.replaceAll('\\', '/'), count: entries.length })}`);
}
...
export async function mergeIdeSettings(ides, targetDir) {
  if (ides.includes('claude-code')) await mergeClaudeSettings(targetDir);
}

// Garante os dois hooks do Citation Gate SEM apagar o que o aluno tem no
// `.claude/settings.json` — que é dele, não nosso: ali moram permissões e hooks
// próprios. Enquanto o merge só protegia vscode/qwen/gemini, este arquivo era
// copiado por cima e o `update` clobberava a configuração do usuário. Com o
// sistema Claude-only ele passou a ser o ÚNICO settings — e o único sem proteção.
// Idempotente: reconhece o hook pelo nome do script, não pela linha inteira.
// Cada gate declara em QUAIS eventos precisa estar. Uma lista, não um `if` por
// hook: o `settings.json` é MESCLADO no `update` (não sobrescrito), então um gate
// que não esteja aqui é copiado para o disco e nunca registrado — nasce órfão,
// e ninguém percebe, porque o arquivo existe e o pipeline segue verde.
const GATES = [
  { script: 'verifica-citacoes.mjs', eventos: ['PreToolUse', 'PostToolUse'] },
  // Detector de vazamento de pipeline: só PreToolUse, porque andaime tem de ser
  // barrado ANTES de virar arquivo — depois de gravado, já é peça com marca de fábrica.
  { script: 'verifica-redacao.mjs', eventos: ['PreToolUse'] },
];

function ensureCitationGateHooks(parsed) {
  if (!parsed.hooks || typeof parsed.hooks !== 'object') parsed.hooks = {};
  for (const gate of GATES) {
    const comando = `node "$CLAUDE_PROJECT_DIR/.claude/hooks/${gate.script}"`;
    const id = gate.script.replace(/\.mjs$/, '');
    for (const evento of gate.eventos) {
      if (!Array.isArray(parsed.hooks[evento])) parsed.hooks[evento] = [];
      // Idempotente pelo NOME do script, não pela linha inteira — o aluno pode ter
      // reescrito o comando (caminho absoluto, wrapper) e continua valendo.
      const jaTem = parsed.hooks[evento].some((entrada) => (entrada?.hooks || [])
        .some((h) => typeof h?.command === 'string' && h.command.includes(id)));
      if (!jaTem) {
        parsed.hooks[evento].push({
          matcher: 'Write|Edit',
...
function mergeClaudeSettings(targetDir) {
  return mergeJsonSetting({
    ...
    templatePath: join(TEMPLATES_DIR, 'ide-templates', 'claude-code', '.claude', 'settings.json'),
    displayName: '.claude/settings.json',
    ensure: ensureCitationGateHooks,
    warnHint: 'O Citation Gate pode não estar ativo — confira os hooks.',
  });
}
